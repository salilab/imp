#include "CoarseCC.h"
 ...
 float CoarseCC::evaluate(const DensityMap &em_map, 
  ...
  float threshold=model_map.get_header()->dmin;
				...
				)
{
  ...
  for (int ii=0; ii<access_p.get_size(); ii++) {
    ...
    dvx[ii] =  access_p.get_w(ii) * 2.*params->get_inv_sigsq() * scalefac * params->get_normfac() * tdvx;
    dvy[ii] =  access_p.get_w(ii) * 2.*params->get_inv_sigsq() * scalefac * params->get_normfac() * tdvy;
    dvz[ii] =  access_p.get_w(ii) * 2.*params->get_inv_sigsq() * scalefac * params->get_normfac() * tdvz;
  }
}
