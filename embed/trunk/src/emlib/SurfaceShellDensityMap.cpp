...
SurfaceShellDensityMap::SurfaceShellDensityMap(
      ...
      int num_shells, float resolution, float voxel_size,
      int sig_cutoff)
      :SampledDensityMap(access_p,resolution,voxel_size,sig_cutoff)
{
  set_parameters();
  set_neighbor_mask();
}
...
void SurfaceShellDensityMap::set_neighbor_mask(emreal delta) {
  for (int x = -1; x <= 1; x++) {
    for (int y = -1; y <= 1; y++) {
      for (int z = -1; z <= 1; z++) {
        if (x == 0 && y == 0 && z == 0) 
        ...
        neighbor_dist_.push_back(delta * sqrt((1.0*x*x + y*y + z*z)));
      }
    }
  }  
}
void SurfaceShellDensityMap::set_parameters() {
  background_val_=-50.0;
  surface_val_=0.0;
}
...
bool SurfaceShellDensityMap::has_background_neighbor(long voxel_ind) {
  ...
  for (unsigned int j = 0; j < neighbor_shift_.size(); j++) {
    ...
    if ((n_voxel_ind>-1) && (n_voxel_ind<num_voxels)) {
      if (data_[n_voxel_ind] == background_val_) {
        return true;
      }
    }
  }
...
void SurfaceShellDensityMap::compute_shells(
            const ParticlesAccessPoint &access_p,
            int num_shells) {

  //all scene voxels will be assigned the value -background_val_ 
  //(which is positive and larger than 0)
  //TODO - change here, the value of the inner voxels should not be 
  //should not be ns*2 but the largest of the inner shell
  binaries(access_p,num_shells*2,background_val_);
  ...
  shell_voxels.insert(
             shell_voxels.end(),header_.nx * header_.ny * header_.nz,-1);
  for(long i=0;i<header_.nx * header_.ny * header_.nz;i++) {
    if (data_[i] == surface_val_) {
      shell_voxels[i]=0;
    }
  }

  long n_voxel_ind,voxel_ind;
  float val; //the value is the distance of the voxel from the surface
  std::vector<long> *curr_p = &curr_shell_voxels;
  std::vector<long> *next_p = &next_shell_voxels;
  std::vector<long> *tmp_p;
  long num_voxels = header_.nx * header_.ny * header_.nz;
  for (int s_ind = 0; s_ind <num_shells; s_ind++) {
