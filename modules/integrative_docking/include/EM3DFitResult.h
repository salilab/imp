...
#ifndef IMPINTEGRATIVE_DOCKING_EM3DFIT_RESULT_H
#define IMPINTEGRATIVE_DOCKING_EM3DFIT_RESULT_H
...
class EM3DFitResult {
 public:
  EM3DFitResult(int number, float score, bool filtered, float cc,
  ...
  // IO
  friend std::ostream& operator<<(std::ostream& s, const EM3DFitResult& p) {
    s.width(6);
    s << p.number_ << " | ";
    s.precision(3);
    s.width(8);
    ...
    s << euler_angles.get_x() << " " << euler_angles.get_y() << " "
      ...
      << " " << translation[2] << " | ";
    euler_angles = IMP::algebra::get_fixed_xyz_from_rotation(
        p.ligand_trans_.get_rotation());
...
struct CompareFitResults {
  bool operator()(const EM3DFitResult& r1, const EM3DFitResult& r2) const {
