...
RotamerLibrary::RotamerLibrary(unsigned angle_step)
  : IMP::base::Object("RotamerLibrary%1%")
...
void RotamerLibrary::read_library_file(const std::string &lib_file_name)
{
  ...
  while ( std::getline(rotf, line) )
  {
    ...
    if ( line_str >> res_name >> phi >> psi >> count >> r1 >> r2
        >> r3 >> r4 >> probability >> chi1 >> chi2 >> chi3 >> chi4 )
    {
      ...
      float probability_thr) const
{
  ...
  while ( prob_so_far < probability_thr && rot_end !=
             library_[res_index][angle_index].end() )
  {
