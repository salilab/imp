...
DensityMap *create_density_map(const algebra::BoundingBox3D &bb,
  ...
  IMP_LOG_TERSE("Created map with dimensions "
                << n[0] << " " << n[1] << " " << n[2] << " and spacing "
                << ret->get_spacing() << std::endl);
  return ret.release();
}
...
MapReaderWriter *create_reader_writer_from_name(std::string name) {
  ...
  if (suf == ".mrc") {
    ...
    IMP_THROW("Unable to determine type for file " << name << " with suffix "
                                                   << suf,
              IOException);
  }
}
...
bool DensityMap::is_xyz_ind_part_of_volume(int ix, int iy, int iz) const {
  return (ix >= 0 && ix < header_.get_nx() && iy >= 0 &&
          iy < header_.get_ny() && iz >= 0 && iz < header_.get_nz());
}
...
bool DensityMap::is_part_of_volume(float x, float y, float z) const {
  ...
  return (x_ind >= 0) && (x_ind < header_.get_nx()) && (y_ind >= 0) &&
         (y_ind < header_.get_ny()) && (z_ind >= 0) &&
...
emreal DensityMap::calcRMS() {
  ...
  IMP_LOG_VERBOSE("in calc RMS meanval:" << meanval << " stdval:" << stdval
                                         << std::endl);
  header_.dmin = get_min_value();
  header_.dmax = get_max_value();
  IMP_LOG_VERBOSE("in calc RMS min:" << header_.dmin << " max:" << header_.dmax
                                     << std::endl);
  meanval /= nvox;
...
void DensityMap::set_origin(float x, float y, float z) {
  header_.set_xorigin(x);
  header_.set_yorigin(y);
  header_.set_zorigin(z);
...
bool DensityMap::same_origin(const DensityMap *other) const {
  if (fabs(get_header()->get_xorigin() - other->get_header()->get_xorigin()) <
          EPS &&
      fabs(get_header()->get_yorigin() - other->get_header()->get_yorigin()) <
          EPS &&
      fabs(get_header()->get_zorigin() - other->get_header()->get_zorigin()) <
          EPS)
    return true;
  return false;
}
...
bool DensityMap::same_voxel_size(const DensityMap *other) const {
  if (fabs(get_header()->get_spacing() - other->get_header()->get_spacing()) <
      EPS)
    return true;
  return false;
}
...
algebra::Vector3D DensityMap::get_centroid(emreal threshold) const {
  ...
  IMP_USAGE_CHECK(threshold < max_val,
                  "The input threshold with value "
                      << threshold
                      << " is higher than the maximum density in the map "
                      << max_val);
...
void DensityMap::add(const DensityMap *other) {
  ...
  for (int iz = 0; iz < (z_top_ind - z_orig_ind); iz++) {
    ...
    for (int iy = 0; iy < (y_top_ind - y_orig_ind); iy++) {
      ...
      other_znxny_ynx =
          other_znxny + (iy + oy_orig_ind) * other->header_.get_nx();
...
void DensityMap::pad(int nx, int ny, int nz, float val) {
  ...
  IMP_USAGE_CHECK(nx >= header_.get_nx() && ny >= header_.get_ny() &&
                      nz >= header_.get_nz(),
...
DensityMap *create_density_map(const algebra::Vector3D &origin, int nx, int ny,
  ...
  ret->update_voxel_size(spacing);
  ret->get_header_writable()->compute_xyz_top();
  IMP_LOG_TERSE("Created map with dimensions " << nx << " " << ny << " " << nz
                                               << " and spacing " << spacing
                                               << std::endl);
  return ret.release();
}
...
/* Surround the density map with an extra set of samples assumed to
   be 0.
*/
inline double get_value(const DensityMap *m, int xi, int yi, int zi) {
  if (xi < 0 || yi < 0 || zi < 0)
    return 0.0;
  else if (xi >= m->get_header()->get_nx() || yi >= m->get_header()->get_ny() ||
           zi >= m->get_header()->get_nz())
    return 0.0;
  else {
...
inline void compute_voxel(const DensityMap *m, const algebra::Vector3D &v,
  ...
  for (unsigned int i = 0; i < 3; ++i) {
    ...
    IMP_INTERNAL_CHECK(remainder[i] < 1.01 && remainder[i] >= -.01,
                       "Algebraic error " << remainder[i] << " " << i << v
                                          << get_bounding_box(m) << " "
                                          << m->get_spacing() << " "
                                          << m->get_origin() << " " << fvox);
  }
...
void get_transformed_internal(const DensityMap *in,
  ...
  if (in->get_header()->get_has_resolution()) {
    ret->get_header_writable()->set_resolution(
        in->get_header()->get_resolution());
  }
  ret->calcRMS();
}
}
...
DensityMap *get_resampled(DensityMap *in, double scaling) {
  ...
  IMP_LOG_TERSE(
      "Resample from "
      << in->get_name() << " with spacing " << in->get_spacing() << " vs "
      << ret->get_spacing() << " and with top " << in->get_top() << " vs "
      << ret->get_top() << " with min/max "
      ...
      << " vs " << *std::min_element(
                        ret->get_data(),
                        ret->get_data() + ret->get_number_of_voxels()) << "..."
      << *std::max_element(ret->get_data(),
                           ret->get_data() + ret->get_number_of_voxels())
      << std::endl);
  IMP_LOG_TERSE("Old map was " << in->get_header()->get_nx() << " "
                               << in->get_header()->get_ny() << " "
                               << in->get_header()->get_nz() << std::endl);
  if (in->get_header()->get_has_resolution()) {
    ret->get_header_writable()->set_resolution(std::max<double>(
        in->get_header()->get_resolution(), ret->get_spacing()));
  }
...
DensityMap *DensityMap::pad_margin(int mrg_x, int mrg_y, int mrg_z,
  ...
  ret->set_origin(get_origin() - header_.get_spacing() *
                                     algebra::Vector3D(mrg_x, mrg_y, mrg_z));
  ...
  for (int iz = 0; iz < header_.get_nz(); iz++) {  // z slowest
    ...
    for (int iy = 0; iy < header_.get_ny(); iy++) {
      ...
      for (int ix = 0; ix < nx; ix++) {
        ...
        IMP_INTERNAL_CHECK(new_ind < new_n_xyz, "Index problem with new map!"
                                                    << std::endl);
        IMP_INTERNAL_CHECK(curr_ind < n_xyz, "Index problem with old map!"
                                                 << std::endl);
...
int DensityMap::lower_voxel_shift(emreal loc, emreal kdist, emreal orig,
  ...
  imin = static_cast<int>(
      std::floor((loc - kdist - orig) / header_.get_spacing()));
  // imin = static_cast<int>(std::floor((loc-kdist) / header_.get_spacing()));
  // bookkeeping
  if (imin < 0) imin = 0;
  if (imin > ndim - 1) imin = ndim - 1;
  return imin;
}
...
float DensityMap::get_maximum_value_in_xy_plane(int z_ind) {
  IMP_USAGE_CHECK(z_ind < header_.get_nz(), "Z index is out of range\n");
...
float DensityMap::get_maximum_value_in_xz_plane(int y_ind) {
  IMP_USAGE_CHECK(y_ind < header_.get_ny(), "Y index is out of range\n");
  float max_val = -INT_MAX;
  int temp_ind;
...
float DensityMap::get_maximum_value_in_yz_plane(int x_ind) {
  IMP_USAGE_CHECK(x_ind < header_.get_ny(), "X index is out of range\n");
  float max_val = -INT_MAX;
  int temp_ind;
  for (int iz = 0; iz < header_.get_nz(); iz++) {
...
IMPEMEXPORT DensityMap *get_segment(DensityMap *from_map, int nx_start,
...
double convolute(const DensityMap *m1, const DensityMap *m2) {
  ...
  int ivoxy_shift =
      (int)floor((h2->get_yorigin() - h1->get_yorigin()) / voxel_size);
  int ivoxz_shift =
      (int)floor((h2->get_zorigin() - h1->get_zorigin()) / voxel_size);
  long j;  // m1 index
  long i;  // m2 index
  long nvox = m1->get_number_of_voxels();
  // calculate the shift in index of the origin of m2 in m1
  // ( j can be negative)
  j = ivoxz_shift * h1->get_nx() * h1->get_ny() + ivoxy_shift * h1->get_nx() +
      ivoxx_shift;
  double conv = 0.;
  for (i = 0; i < nvox; i++) {
    if (j + i >= 0 && j + i < nvox) {
...
DensityMap *get_max_map(DensityMaps maps) {
  ...
  for (long ind = 0; ind < nx * ny * nz; ind++) {
    for (DensityMaps::const_iterator it = maps.begin(); it != maps.end();
         it++) {
...
DensityMap *get_segment(DensityMap *map_to_segment, algebra::Vector3Ds vecs,
                        float radius) {
  ...
  for (int i = 0; i < (int)vecs.size(); i++) {
    ...
    calc_local_bounding_box(map_to_segment, vecs[i][0], vecs[i][1], vecs[i][2],
                            radius, iminx, iminy, iminz, imaxx, imaxy, imaxz);
    for (ivoxz = iminz; ivoxz <= imaxz; ivoxz++) {
      ...
      if (count < 3)
        std::cout << "Particle " << i << " has no data" << std::endl;
    }
  }
  return ret.release();
}
        ...
        if (val > EPS) {  // smooth this value
          for (int iz2 = -margin; iz2 <= static_cast<int>(margin); iz2++) {
            for (int iy2 = -margin; iy2 <= static_cast<int>(margin); iy2++) {
              kernel_ind =
                  (iz2 + margin) * dim_len * dim_len + (iy2 + margin) * dim_len;
              for (int ix2 = -margin; ix2 <= static_cast<int>(margin); ix2++) {
                data_[(iz + iz2) * ny * nx + (iy + iy2) * nx + ix + ix2] +=
...
DensityMap *interpolate_map(DensityMap *in_map, double new_spacing) {
  ...
  int round_ret_top_y =
      floor((in_orig[1] + in_spacing * (in_ny - 1)) / new_spacing);
  int round_ret_top_z =
      floor((in_orig[2] + in_spacing * (in_nz - 1)) / new_spacing);
  // update grid size
          ...
          for (int iz2 = -shell_w; check == 0 && iz2 <= shell_w; iz2++)
            for (int iy2 = -shell_w; check == 0 && iy2 <= shell_w; iy2++)
              for (int ix2 = -shell_w; check == 0 && ix2 <= shell_w; ix2++) {
                if (ix + ix2 < (int)nx && iy + iy2 < (int)ny &&
                    iz + iz2 < (int)nz && ix + ix2 >= 0 && iy + iy2 >= 0 &&
                    iz + iz2 >= 0) {
