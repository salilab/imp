...
void FittingSolutions::add_solution(const algebra::Transformation3D &t,
                                    Float score) {
...
kernel::RestraintSet *add_restraints(kernel::Model *model, DensityMap *dmap,
                                     kernel::Particle *p,
                                     kernel::Refiner *leaves_ref,
                                     const FloatKey &wei_key,
                                     bool fast = false) {
  kernel::RestraintSet *rsrs =
      new kernel::RestraintSet(model, 1.0, "rigid fitting restraints %1%");
...
core::MonteCarlo *set_optimizer(kernel::Model *model,
...
void optimize(Int number_of_optimization_runs, Int number_of_mc_steps,
              const algebra::Vector3D &anchor_centroid, kernel::Particle *p,
              Refiner *refiner, core::MonteCarlo *opt, FittingSolutions &fr,
              kernel::Model *) {
  Float e;
  core::RigidBody rb =
      core::RigidMember(refiner->get_refined(p)[0]).get_rigid_body();
...
FittingSolutions local_rigid_fitting_around_point(
    ...
    OptimizerStates display_log, Int number_of_optimization_runs,
    Int number_of_mc_steps, Int number_of_cg_steps, Float max_translation,
    Float max_rotation, bool fast) {
  FittingSolutions fr;
  IMP_LOG_TERSE(
      "rigid fitting with "
      << number_of_optimization_runs << " MC optimization runs, each with "
      << number_of_mc_steps << " Monte Carlo steps , each with "
      << number_of_cg_steps << " Conjugate Gradients rounds. " << std::endl
  ...
  optimize(number_of_optimization_runs, number_of_mc_steps, anchor_centroid, p,
           refiner, opt, fr, model);
...
FittingSolutions local_rigid_fitting_around_points(
    ...
    Float max_rotation) {
  FittingSolutions fr;
  IMP_LOG_VERBOSE("rigid fitting around "
                  << anchor_centroids.size() << " with "
  ...
  kernel::RestraintSet *rsrs = add_restraints(model, dmap, p, refiner, wei_key);
  base::PointerMember<core::MonteCarlo> opt =
      set_optimizer(model, display_log, p, refiner, number_of_cg_steps,
                    max_translation, max_rotation);
  ...
  for (algebra::Vector3Ds::const_iterator it = anchor_centroids.begin();
    ...
    optimize(number_of_optimization_runs, number_of_mc_steps, *it, p, refiner,
             opt, fr, model);
  }
  fr.sort();
  model->remove_restraint(rsrs);
...
FittingSolutions local_rigid_fitting_grid_search(
    ...
    Int max_voxels_translation, Int translation_step,
    Float max_angle_in_radians, Int number_of_rotations) {
  FittingSolutions fr;
  IMP_USAGE_CHECK(
      ps.size() > 0,
  ...
  IMP_INTERNAL_CHECK(model_dens_map->same_dimensions(dmap),
  ...
  for (algebra::Rotation3Ds::iterator it = rots.begin(); it != rots.end();
       it++) {
    ++rot_ind;
    IMP_LOG_TERSE("working on rotation " << rot_ind << " out of " << rots.size()
                                         << std::endl);
...
FittingSolutions compute_fitting_scores(
    const kernel::ParticlesTemp &ps, DensityMap *em_map,
    const algebra::Transformation3Ds &transformations, bool fast_version,
    bool local, const FloatKey &wei_key) {
  FittingSolutions fr;
  // move the particles to the center of the map
  algebra::Transformation3D move_ps_to_map_center = algebra::Transformation3D(
  ...
  if (!fast_version) {
    ...
    IMP_LOG_VERBOSE("model_dens_map2 rms:" << model_dens_map2->get_header()->rms
                                           << " particles centroid: "
    ...
    for (algebra::Transformation3Ds::const_iterator it = trans_for_fit.begin();
      ...
      model_dens_map2->resample();
      model_dens_map2->calcRMS();
      IMP_LOG_VERBOSE("model_dens_map2 rms:"
                      << model_dens_map2->get_header()->rms
                      << " particles centroid: "
                      << core::get_centroid(core::XYZs(ps)) << "trans: " << *it
                      << std::endl);
    ...
    IMP_LOG_VERBOSE("running fast version of compute_fitting_scores"
                    << std::endl);
    for (algebra::Transformation3Ds::const_iterator it = trans_for_fit.begin();
         it != trans_for_fit.end(); it++) {
      ...
      IMP_INTERNAL_CHECK(transformed_sampled_map->same_dimensions(em_map),
                         "sampled density map is of wrong dimensions"
                             << std::endl);
...
Float compute_fitting_score(const kernel::ParticlesTemp &ps, DensityMap *em_map,
                            FloatKey wei_key) {
