...
void RadiusDependentKernelParameters::show(std::ostream &s) const {
  s << "vsig : " << vsig_ << " vsigsq: " << vsigsq_
    << " inv_sigsq: " << inv_sigsq_ << " sig: " << sig_ << " kdist: " << kdist_
    << " normfac: " << normfac_ << std::endl;
}
RadiusDependentKernelParameters::RadiusDependentKernelParameters(
    float radii, float rsigsq, float timessig, float sq2pi3, float inv_rsigsq,
...
KernelParameters::~KernelParameters() {
  for (std::map<float, const RadiusDependentKernelParameters *>::iterator it =
           radii2params_.begin();
...
const RadiusDependentKernelParameters &KernelParameters::set_params(
    float radius) {
  ...
  IMP_USAGE_CHECK(iter == radii2params_.end(),
                  "The Kernel Parameters for the radius "
                      << radius << " have already been calculated");
...
const RadiusDependentKernelParameters &KernelParameters::get_params(
  ...
  IMP_USAGE_CHECK(initialized_, "The Kernel Parameters are not initialized");
  typedef std::map<float, const RadiusDependentKernelParameters *> kernel_map;
  ...
  if (algebra::get_are_almost_equal(radius, upper_closest->first, eps)) {
    ...
    IMP_LOG_VERBOSE("for radius:" << radius << " the closest is:"
                                  << upper_closest->first << std::endl);
  } else {
