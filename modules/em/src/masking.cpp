  ...
  for (int x = -delta; x <= delta; x++) {
    for (int y = -delta; y <= delta; y++) {
      for (int z = -delta; z <= delta; z++) {
        ...
        neighbor_shift_.push_back(z * header.get_nx() * header.get_ny() +
                                  y * header.get_nx() + x);
...
const RadiusDependentDistanceMask *DistanceMask::set_mask(
  ...
  IMP_USAGE_CHECK(initialized_, "distance mask is not initialized\n");
  const RadiusDependentDistanceMask *mask = get_mask(sampling_radius);
  IMP_USAGE_CHECK(mask == nullptr, "mask for radius "
                                       << sampling_radius
                                       << " has already been calculated \n");
  IMP_LOG_TERSE("generating a distance mask for radius:" << sampling_radius
                                                         << "\n");
...
const RadiusDependentDistanceMask *DistanceMask::get_mask(float radius,
                                                          float eps) const {
