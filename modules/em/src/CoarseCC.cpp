...
double cross_correlation_coefficient_internal(const DensityMap *grid1,
  ...
  if (same_origin) {  // Fastest version
    ...
    int ivoxx_shift =
        (int)floor((grid2_header->get_xorigin() - grid1_header->get_xorigin()) /
                   voxel_size);
    int ivoxy_shift =
        (int)floor((grid2_header->get_yorigin() - grid1_header->get_yorigin()) /
                   voxel_size);
    int ivoxz_shift =
        (int)floor((grid2_header->get_zorigin() - grid1_header->get_zorigin()) /
                   voxel_size);
  ...
  IMP_INTERNAL_CHECK(num_elements > 0,
                     "No voxels participated in the calculation"
                         ...
                         << grid2_voxel_data_threshold << " is off"
                         << std::endl);
  if ((norm_factors.first > 0.) && (norm_factors.second > 0.)) {
    IMP_LOG_VERBOSE(" with norm factors: start ccc : "
                    << ccc << " first norm factor: " << norm_factors.first
                    << " second norm factor: " << norm_factors.second
                    << std::endl);
  ...
  IMP_LOG_VERBOSE(" ccc : " << ccc << " voxel# " << nvox
                            ...
                            << "  " << grid2_header->rms
                            << " means(grid1,grid2) " << grid1_header->dmean
                            << " " << grid2_header->dmean << std::endl);
  return ccc;
}
}
