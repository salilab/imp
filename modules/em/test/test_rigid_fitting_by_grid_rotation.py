...
class Tests(IMP.test.TestCase):
    ...
    def load_density_map(self):
        ...
        self.scene = IMP.em.read_map(
            self.get_input_file_name("1z5s_10.mrc"),
            mrw)
        self.resolution = 10.
        self.voxel_size = 2.
    ...
    def test_cc_after_grid_rotation(self):
        ...
        for p in self.particles:
            IMP.core.XYZ(p).set_coordinates(
                t.get_transformed(IMP.core.XYZ(p).get_coordinates()))
        transformed_density = IMP.em.get_transformed(self.scene,
                                                     t, threshold)
        sampled_density_map1 = IMP.em.SampledDensityMap(
            transformed_density.get_header())
        ...
        # because rotation the grid changes the density a bit
    ...
    def test_cc_with_sampled_grid_rotation(self):
        ...
        # rotate the map
        threshold = sampled_density_map.get_header().dmin
        transformed_sampled_density = IMP.em.get_transformed(
            sampled_density_map, t)
