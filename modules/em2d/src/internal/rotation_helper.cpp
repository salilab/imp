...
void semispherical_spiral_distribution(const unsigned long N,
                              algebra::SphericalVector3Ds &vs,double r) {
  vs.resize(N);
...
void spherical_spiral_distribution(const unsigned long N,
                         algebra::SphericalVector3Ds &vs,double radius) {
  vs.resize(N);
  double zenith,azimuth;
  for (unsigned long k=1;k<=N;++k) {
...
void semispherical_even_distribution(const unsigned long N,
  ...
  for(unsigned long i=0;i<2*N;++i) {
    if(temp[i][1]<(PI/2)) { vs.push_back(temp[i]); }
  }
  // Check
  if(vs.size()<N) {
...
int next(int axis) {
  axis+=1; if(axis>3) { return 1; } else { return axis; }
}

int previous(int axis) {
  axis-=1;  if(axis==0) { return 3; } else { return axis; }
}
