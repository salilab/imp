...
void create_projections(const std::vector<IMP::algebra::Vector3D>& points,
  ...
  for (unsigned int i = 0; i < spherical_coords.size(); i++) {
    ...
    std::auto_ptr<Projection> p(
        new Projection(rotated_points, pixel_size, resolution, axis_size));
    p->set_rotation(r);
...
void create_projections(
    const std::vector<IMP::algebra::Vector3D>& all_points,
    const std::vector<IMP::algebra::Vector3D>& ligand_points,
    unsigned int projection_number, double pixel_size, double resolution,
    boost::ptr_vector<Projection>& projections, int image_size) {
  ...
  for (unsigned int i = 0; i < spherical_coords.size(); i++) {
    ...
    // project
    std::auto_ptr<Projection> p(
        new Projection(rotated_points, rotated_ligand_points, pixel_size,
