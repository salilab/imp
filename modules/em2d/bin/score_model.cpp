...
po::variables_map get_parameters(int argc, char **argv) {
  ...
  desc.add_options()("help",
                     "This is the help. Variables with * are mandatory")(
      "mod", po::value<str>(), "* PDB file with model")(
      "subjs", po::value<str>(), "* File with the names of the subject images")(
      "apix", po::value<double>(),
      "* Pixel size of subjects in Angstroms/pixel")(
      "projs", po::value<str>(),
      "* Way of generating projections. \"model\" for "
      ...
      "n_opt", po::value<unsigned int>()->default_value(10),
      "Optimization steps "
      "for  Simplex")("fast", po::value<unsigned int>()->default_value(0),
                      "Fast mode. Optimize "
...
int main(int argc, char **argv) {
  ...
  IMP_LOG_TERSE("Reading EM subject images from " << fn_subjs << std::endl);
  ...
  if (opt[0] == "model") {
    ...
    IMP_LOG_TERSE("Generating " << n_projections << " projections using model "
                                << fn_model << std::endl);
    ...
    if (save_images) {
      IMP_LOG_TERSE("Saving " << n_projections << " projections " << std::endl);
  ...
  *out << "# Total preprocessing Time: "
       << time_preprocess_subjects + time_preprocess_projections << std::endl;

  if (n_coarse_results_optimized != 0) {
    *out << "# Set fast mode, use " << n_coarse_results_optimized
  ...
  if (registration_option == COMPLETE_REGISTRATION) {
    ...
    *out << "# Fine registration Time: " << finder->get_fine_registration_time()
         << std::endl;
  ...
  *out << "# Registration: images " << subjects.size() << " projections "
       << projections.size() << " Total time: " << total_time << std::endl;
  ...
  *out << "Global result>>" << fn_model << c << resolution << c << apix << c
       << fn_subjs << c << Score << c << total_time << c << n_subjects;
  ...
  *out << std::endl;
  ...
  if (vm.count("bm")) {
    ...
    *out << "# N.projections Score Av.Rot.Error  Av.Shift.Error" << std::endl;
    double rot_error =
        get_average_rotation_error(correct_RRs, registration_results);
    double shift_error =
        get_average_shift_error(correct_RRs, registration_results);
    char c = '|';
    *out << n_projections << c << Score << c << rot_error << c << shift_error
         << std::endl;
  }
  ofs.close();
  return 0;
