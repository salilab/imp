...
double MembraneRestraint::score(double prob) const { return -log(prob); }

double MembraneRestraint::probability_above(double z,
...
double MembraneRestraint::score_above(double z,
                                      double z_slope_center_upper) const {
  return score(probability_above(z, z_slope_center_upper));
}

double MembraneRestraint::probability_below(double z,
                                            double z_slope_center_lower) const {
  double const argvalue((z - z_slope_center_lower) / softness);
  return (1.0 - plateau) / (1.0 + exp(argvalue));
}
double MembraneRestraint::score_below(double z,
                                      double z_slope_center_lower) const {
  return score(probability_below(z, z_slope_center_lower));
}

double MembraneRestraint::score_inside(double z, double z_slope_center_lower,
                                       double z_slope_center_upper) const {
  return score(1.0 - probability_above(z, z_slope_center_upper)) +
         score(1.0 - probability_below(z, z_slope_center_lower));
}
...
double MembraneRestraint::unprotected_evaluate(DerivativeAccumulator *) const {
  ...
  for (size_t i(0); i < particles_below.size(); ++i) {
    sb += score_below(core::XYZ(m, particles_below[i]).get_z(),
                      z_slope_center_lower);
  }
  double sa(0.0);
  for (size_t i(0); i < particles_above.size(); ++i) {
    sa += score_above(core::XYZ(m, particles_above[i]).get_z(),
                      z_slope_center_upper);
  }
  double si(0.0);
  for (size_t i(0); i < particles_inside.size(); ++i) {
    si += score_inside(core::XYZ(m, particles_inside[i]).get_z(),
