...
void Model::remove_particle(Particle *p) { remove_particle(p->get_index()); }
  ...
  IMP_IF_CHECK(base::USAGE) {
    for (unsigned int i = 0; i < internal::ParticleAttributeTable::size();
      ...
      for (unsigned int j = 0; j < internal::ParticleAttributeTable::size(i);
           ++j) {
        if (internal::ParticleAttributeTable::get_has_attribute(
                ParticleIndexKey(i), ParticleIndex(j))) {
          ParticleIndex pc = internal::ParticleAttributeTable::get_attribute(
              ParticleIndexKey(i), ParticleIndex(j), false);
    ...
    for (unsigned int i = 0; i < internal::ParticlesAttributeTable::size();
      ...
      for (unsigned int j = 0; j < internal::ParticlesAttributeTable::size(i);
           ++j) {
        if (internal::ParticlesAttributeTable::get_has_attribute(
                ParticleIndexesKey(i), ParticleIndex(j))) {
          ParticleIndexes pcs =
              internal::ParticlesAttributeTable::get_attribute(
                  ParticleIndexesKey(i), ParticleIndex(j), false);
...
void Model::remove_data(ModelKey mk) { model_data_[mk.get_index()] = nullptr; }
