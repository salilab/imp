...
ContainersTemp ParticleInputs::get_input_containers(Particle *p) const {
  IMPKERNEL_DEPRECATED_FUNCTION_DEF(2.1, "Call get_inputs instead.");
  return IMP::kernel::get_input_containers(get_inputs(p->get_model(),
                                          ParticleIndexes(1,
                                                         p->get_index())));
...
ParticlesTemp ParticleInputs::get_input_particles(Particle *p) const {
  IMPKERNEL_DEPRECATED_FUNCTION_DEF(2.1, "Call get_inputs instead.");
  return IMP::kernel::get_input_particles(get_inputs(p->get_model(),
                                         ParticleIndexes(1,
                                                         p->get_index())));
}

ModelObjectsTemp
ParticleInputs::do_get_inputs(Model *m,
                              const ParticleIndexes &pis) const {
...
ModelObjectsTemp ParticleOutputs::get_outputs(Model *m,
                                            const ParticleIndexes &pis) const {
  return do_get_outputs(m, pis);
}

ContainersTemp ParticleOutputs::get_output_containers(Particle *p) const {
  IMPKERNEL_DEPRECATED_FUNCTION_DEF(2.1, "Call get_outputs instead.");
  return IMP::kernel::get_output_containers(get_outputs(p->get_model(),
                                           ParticleIndexes(1,
                                                           p->get_index())));
...
ParticlesTemp ParticleOutputs::get_output_particles(Particle *p) const {
  IMPKERNEL_DEPRECATED_FUNCTION_DEF(2.1, "Call get_outputs instead.");
  return IMP::kernel::get_output_particles(get_outputs(p->get_model(),
                                         ParticleIndexes(1,
                                                         p->get_index())));
}

ModelObjectsTemp
ParticleOutputs::do_get_outputs(Model *m,
                              const ParticleIndexes &pis) const {
  ModelObjectsTemp ret;
  for (unsigned int i = 0; i < pis.size(); ++i) {
...
get_required_score_states(const ModelObjectsTemp &mos,
  ...
  std::set_difference(ret.begin(), ret.end(),
