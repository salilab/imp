  ...
  class PyOutFileAdapter : public IMP::Object
{
  ...
  IMP_UNIQUE_PTR<std::ostream> ostr_;
  ...
  IMP_UNIQUE_PTR<StreamBuf> stream_buf_;
public:
 PyOutFileAdapter():IMP::Object("PyOutFileAdapter") {
  ...
  std::ostream* set_python_file(PyObject *p) {
    ...
    stream_buf_= IMP_UNIQUE_PTR<StreamBuf>(new StreamBuf(wm));

    IMP_INTERNAL_CHECK(!ostr_.get(), "Already set the stream.");
    ostr_ = IMP_UNIQUE_PTR<std::ostream>(new std::ostream(stream_buf_.get()));
   ...
   class PyInFileAdapter: public IMP::Object
{
  ...
  IMP_UNIQUE_PTR<InAdapter> streambuf_;
  IMP_UNIQUE_PTR<std::istream> istr_;
public:
...
#if PY_VERSION_HEX < 0x03000000
    if (real_file) {
      streambuf_ = IMP_UNIQUE_PTR<InAdapter>(new PyInCFileAdapter(PyFile_AsFile(p)));
    } else 
#endif
    {
      ...
      streambuf_ = IMP_UNIQUE_PTR<InAdapter>(new PyInFilelikeAdapter(read_method));
    }
    IMP_INTERNAL_CHECK(!istr_.get(), "Already set the stream.");
    istr_ = IMP_UNIQUE_PTR<std::istream>(new std::istream(streambuf_.get()));
