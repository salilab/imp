/**
 *  \file IMP/VectorD.h   \brief Simple D vector class.
 *
 *  Copyright 2007-2026 IMP Inventors. All rights reserved.
 *
 */

#ifndef IMPKERNEL_VECTOR_D_H
#define IMPKERNEL_VECTOR_D_H
 ...
 */
template <int D>
class VectorD : public internal::VectorBaseD<D> {
  /* implementing it via a specialization is in order to get swig
     to only use the right constructors as well as C++. */
  IMP_VECTOR_METHODS(D);
};

template <>
class VectorD<-1> : public internal::VectorBaseD<-1> {
...
template <>
class VectorD<2> : public internal::VectorBaseD<2> {
 public:
  IMP_VECTOR_METHODS(2);
...
template <>
class VectorD<3> : public internal::VectorBaseD<3> {
 public:
  IMP_VECTOR_METHODS(3);
...
template <>
class VectorD<4> : public internal::VectorBaseD<4> {
 public:
  IMP_VECTOR_METHODS(4);
...
template <>
class VectorD<5> : public internal::VectorBaseD<5> {
 public:
  IMP_VECTOR_METHODS(5);
...
template <>
class VectorD<6> : public internal::VectorBaseD<6> {
 public:
  IMP_VECTOR_METHODS(6);
...
/** \see VectorD */
template <int D>
inline VectorD<D> get_elementwise_product(const VectorD<D> &a,
                                          const VectorD<D> &b) {
  VectorD<D> ret(a);
...
namespace std {
  ...
  const double& get(const IMP::VectorD<D> &v) {
