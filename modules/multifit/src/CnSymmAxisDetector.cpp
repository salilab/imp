...
void CnSymmAxisDetector::init_from_density(IMP::em::DensityMap *dmap,
  ...
  //  std::cout<<"Taking "<<vecs_.size()<<" vectors with
  //  density higher than :"<<top_p_den_val<<std::endl;

  //calculate  pca
  pca_= algebra::get_principal_components(vecs_);
  //calculate transformation from the native axes system to
  //the one defined by the pca
...
float CnSymmAxisDetector::calc_symm_score(int symm_axis_ind) const {
  ...
  IMP_LOG(TERSE,"average STD from "<<avg_std_count<<
