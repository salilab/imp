...
algebra::Transformation3Ds AlignSymmetric::
         generate_cn_density_translations() const{
...
AlignSymmetric::AlignSymmetric(em::DensityMap *dmap,float dens_t,
  ...
  symm_map_ = new CnSymmAxisDetector(cn_symm_deg_);
  symm_map_->init_from_density(dmap,dens_t,0.0);
...
int AlignSymmetric::score_alignment(atom::Hierarchies mhs,
  ...
  multifit::MolCnSymmAxisDetector symm_mol(cn_symm_deg_);
  symm_mol.init_from_protein(mhs);
  ...
  IMP_LOG(VERBOSE,"MAP :"<<map_v_[0]<<","<<map_v_[1]<<","<<map_v_[2]<<
          " MOL :" <<mol_v[0]<<","<<mol_v[1]<<","<<mol_v[2]<<
          " counter:"<<count<<" dn_symm_deg:"<<dn_symm_deg_<<
