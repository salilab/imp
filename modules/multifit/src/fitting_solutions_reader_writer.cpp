...
FittingSolutionRecord parse_fitting_line(const std::string &line) {
  ...
  std::vector<std::string> line_split, fit_rotation_split,
      fit_translation_split, dock_rotation_split, dock_translation_split;
  ...
  IMP_USAGE_CHECK(fit_rotation_split.size() == 4,
                  "Wrong format of input line: wrong rotation format "
                      << "(expected 4 blocks and got "
                      << fit_rotation_split.size() << ")" << std::endl);
  ...
  IMP_USAGE_CHECK(fit_translation_split.size() == 3,
                  "Wrong format of input line: wrong translation format"
                      << std::endl);
  ...
  fit_sol.set_fit_transformation(algebra::Transformation3D(
      algebra::Rotation3D(boost::lexical_cast<float>(fit_rotation_split[0]),
  ...
  fit_sol.set_dock_transformation(algebra::Transformation3D(
      algebra::Rotation3D(boost::lexical_cast<float>(dock_rotation_split[0]),
...
FittingSolutionRecords read_fitting_solutions(const char *fitting_fn) {
  ...
  if (!in.good()) {
    IMP_WARN("Problem opening file " << fitting_fn
                                     << " for reading; returning 0 solutions"
                                     << std::endl);
    in.close();
    return sols;
  }
...
FittingSolutionRecords convert_transformations_to_multifit_format(
...
algebra::Transformation3Ds convert_multifit_format_to_transformations(
