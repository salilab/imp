...
boost::tuple<std::string, std::string, IntsList> parse_protein_line(
    const std::string &config, const std::string &line, int max_paths) {
  IMP_USAGE_CHECK(line.size() > 0,
                  "no data to parse for protein line:" << line << std::endl);
  IMP_LOG_VERBOSE("going to parse:" << line);
  std::vector<std::string> line_split;
  boost::split(line_split, line, boost::is_any_of("|"));
  // split returns zero length entires as well
