...
ComponentHeader parse_component_line(const std::string &line) {
  try {
  ...
  comp.set_filename(boost::lexical_cast<std::string>(line_split[1]));
  comp.set_pdb_ap_fn(boost::lexical_cast<std::string>(line_split[2]));
  comp.set_num_ap(boost::lexical_cast<int>(line_split[3]));
  comp.set_transformations_fn(boost::lexical_cast<std::string>(line_split[4]));
  comp.set_reference_fn(boost::lexical_cast<std::string>(line_split[5]));
  return comp;
  }
...
AssemblyHeader parse_assembly_line(const std::string &line) {
  typedef boost::split_iterator<std::string::iterator> string_split_iterator;
  IMP_USAGE_CHECK(line.size() > 0,"no data to parse"<<std::endl);
  IMP_LOG(VERBOSE,"going to parse:"<<line);
  std::vector<std::string> line_split;
...
SettingsData read_settings(const char *filename) {
  ...
  while (!in.eof()) {
    ...
    if ((line_split.size() == 7) && (status == 0)) {//protein  line
      header.add_component_header(parse_component_line(line));
    }
    else if (status==0) {//map header line
      status=1;
    }
    else if (status==1){ //map line
      header.set_assembly_header(parse_assembly_line(line));
      status=2;
    }
    else {//(status == 2)
