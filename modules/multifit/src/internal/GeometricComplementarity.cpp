...
class SurfaceDistanceMap : public IMP::em::SampledDensityMap {
 public:
...
void SurfaceDistanceMap::create_distances(
    const std::vector<long> &surface_voxels, std::vector<int> &shell_voxels) {
...
IMP::algebra::DenseGrid3D<float> get_complementarity_grid(
  ...
  IMP_GRID3D_FOREACH_VOXEL(
      ...
      long idx = sdm->get_voxel_by_location(voxel_center[0], voxel_center[1],
                                            voxel_center[2]);
      if (idx > -1) {
        ...
        if (v > 0) {
          ...
          **/
          ...
          Floats::const_iterator p =
              std::lower_bound(params.complementarity_thickness.begin(),
...
FitScore get_fit_scores(const Grid &map0, const Grid &map1,
