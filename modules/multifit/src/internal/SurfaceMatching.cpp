...
void store_in_hash(const IMP::algebra::DenseGrid3D<float> &map,
  ...
  IMP_GRID3D_FOREACH_VOXEL(map, if (map[voxel_center] == 1) {
...
unsigned int get_inside_voxels(const InsideHash &inside,
  ...
  InsideHash::HashResult points =
      inside.neighbors(InsideHash::EUCLIDEAN, pt.first, radius);
  PointLabel &pt_data = pt.second;
  // now calculate center of mass
  IMP::algebra::Vector3D center(0, 0, 0);
  for (unsigned int i = 0; i < points.size(); ++i) center += points[i]->first;
...
CriticalPointType classify_voxel(BoundaryHash &boundary,
  ...
  if (pt_data.shape_value > 2.0 / 3.0) {
    ...
    return OTHER;
  BoundaryHash::HashResult points =
      boundary.neighbors(BoundaryHash::EUCLIDEAN, pt.first, radius);
  if (points.size() < N_NEAREST_POINTS + 1) return OTHER;
  std::vector<HashPointData> point_distances(points.size());
  for (unsigned int i = 0; i < points.size(); ++i) {
    point_distances[i].index = i;
    point_distances[i].distance =
        IMP::algebra::get_squared_distance(pt.first, points[i]->first);
  }
  ...
  for (unsigned int i = 1; i <= N_NEAREST_POINTS; ++i) {
    ...
    if (candidate == HOLE) {
      if (other_shape_value > pt_data.shape_value) return OTHER;
    } else {
      if (other_shape_value < pt_data.shape_value) return OTHER;
    }
  }
...
void calculate_shape_function(const InsideHash &inside, BoundaryHash &boundary,
                              double radius, double unit_volume) {
...
double get_torsion(const IMP::algebra::Vector3D &unit0,
                   ...
                   const IMP::algebra::Vector3D &unit2) {
  IMP::algebra::Vector3D cross12 = get_vector_product(unit1, unit2);
  return std::atan2(unit0 * cross12,
                    get_vector_product(unit0, unit1) * cross12);
}
...
bool match_angles(const BoundaryHash::ValueType &critical0,
                  const BoundaryHash::ValueType &critical1,
                  double max_normal_error, double &normal_error_sum,
                  double &torsion_sum) {
  ...
  if (normal_error > max_normal_error) return false;
  normal_error_sum += normal_error;
  double torsion =
      get_torsion(critical0.second.normal, from0_to_1, critical1.second.normal);
...
get_candidate_transformations_match_1st_surface_to_2nd(
  ...
  IMP_LOG_VERBOSE(__FUNCTION__ << ": splitting map0 into boundary and inside"
                               << std::endl);
  store_in_hash(map0, inside0, boundary0);
  IMP_LOG_VERBOSE(__FUNCTION__ << ": calculating the shape function of map0"
                               << std::endl);
  ...
  IMP_LOG_VERBOSE(__FUNCTION__ << ": map0(knobs: " << knobs0.size()
                               << ", holes:" << holes0.size() << ")\n");
  InsideHash inside1(ball_radius);
  BoundaryHash boundary1(ball_radius);
  IMP_LOG_VERBOSE(__FUNCTION__ << ": splitting map1 into boundary and inside"
                               << std::endl);
  store_in_hash(map1, inside1, boundary1);
  IMP_LOG_VERBOSE(__FUNCTION__ << ": calculating the shape function of map1"
                               << std::endl);
  ...
  IMP_LOG_VERBOSE(__FUNCTION__ << ": map1(knobs: " << knobs1.size()
                               << ", holes:" << holes1.size() << ")"
                               << std::endl);
  for (size_t i = 0; i < knobs0.size(); ++i) {
    ...
    for (size_t j = 0; j < holes0.size(); ++j) {
      ...
      if (dist0 > params.get_maximum_pair_distance()) continue;
      for (size_t k = 0; k < knobs1.size(); ++k) {
        ...
        for (size_t l = 0; l < holes1.size(); ++l) {
          ...
          if (dist1 > params.get_maximum_pair_distance()) continue;
          ...
          if (std::fabs(torsion_sum0 - torsion_sum1) >
            ...
            continue;
          if (normal_error_sum > params.get_maximum_normal_error()) continue;
          ...
          result.push_back(
              IMP::algebra::get_transformation_aligning_first_to_second(from,
                                                                        to));
        }
      }
    }
