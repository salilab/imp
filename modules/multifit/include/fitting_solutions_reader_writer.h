...
IMPMULTIFITEXPORT FittingSolutionRecords
    read_fitting_solutions(const char *fitting_fn);
...
IMPMULTIFITEXPORT void write_fitting_solutions(
    const char *fitting_fn, const FittingSolutionRecords &fit_sols,
    int num_sols = -1);

IMPMULTIFITEXPORT FittingSolutionRecords
    convert_em_to_multifit_format(const em::FittingSolutions &em_fits);
