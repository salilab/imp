...
public:
  ...
  HashResult neighbors(Distance dt, const Point &center,
      double radius) const
  {
    if ( dt == EUCLIDEAN )
  ...
  void neighbors(Distance dt, const Point &center,
      double radius, HashResultT &values)
  {
    values.clear();
    HashResult nb = neighbors(dt, center, radius);
    values.reserve(nb.size());
    for ( size_t i=0; i<nb.size(); ++i )
      values.push_back(nb[i]->second);
  }
