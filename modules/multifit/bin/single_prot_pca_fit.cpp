...
int parse_input(int argc, char *argv[], std::string &density_filename,
                float &spacing, float &x_origin, float &y_origin,
                float &z_origin, float &resolution, float &threshold,
  ...
  required_params.add_options()("density",
      ...
      "threshold", po::value<float>(&threshold), "density threshold")(
  ...
  if (!(vm.count("threshold") + vm.count("density") + vm.count("apix") +
            vm.count("res") + vm.count("protein") ==
        5)) {
...
int main(int argc, char **argv) {
  ...
  std::string protein_filename, ref_filename, output_filename, sol_filename,
      pdb_fit_filename, log_filename;
  if (parse_input(argc, argv, density_filename, spacing, x_origin, y_origin,
                  z_origin, resolution, threshold, protein_filename,
                  ref_filename, sol_filename, log_filename, pdb_fit_filename)) {
    exit(0);
  }
  // load the density
  em::DensityMap *full_dmap;
  em::DensityMap *dmap;  // resampled
  full_dmap = set_map(density_filename, resolution, spacing, x_origin, y_origin,
                      z_origin);
  ...
  log_file << "origin : (" << x_origin << "," << y_origin << "," << z_origin
           << ")" << std::endl;
  ...
  for (int i = 0; i < num_sols; i++) {
    ...
    core::transform(rb, sols.get_transformation(i));
    if (pdb_fit_filename != "") {
    ...
    core::transform(rb, sols.get_transformation(i).get_inverse());
