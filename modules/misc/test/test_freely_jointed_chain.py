...
def _get_linear_score(z, N, b):
    ...
    intercept = 10.396310953781 - .5 * math.log(_get_beta(N, b))
    return slope * z + intercept


def _get_linear_derv(N, b):
    return -199.98 * _get_beta(N, b)**.5


class Tests(IMP.test.TestCase):

    """Tests for FreelyJointedChain."""

    def test_init(self):
        """Test correct initialization."""
        func = IMP.misc.FreelyJointedChain(10, 8)
    ...
    def test_evaluate(self):
        ...
        for i in range(100):
            ...
            func = IMP.misc.FreelyJointedChain(N, b)
            func.set_was_used(True)
            max_z = 2 * N * b
            beta = _get_beta(N, b)
            min_z = .01 / beta**.5
