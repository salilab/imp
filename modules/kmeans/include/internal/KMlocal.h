...
#ifndef IMPKMEANS_INTERNAL_KMLOCAL_H
#define IMPKMEANS_INTERNAL_KMLOCAL_H
...
class IMPKMEANSEXPORT KMlocalLloyds : public KMlocal {
 ...
 protected:
  ...
  virtual void printStageStats() override {  // print end of stage info
    if (kmStatLev >= STAGE) {
      *kmOut << "\t<stage: " << stageNo << " curr: " << curr.getAvgDist()
 ...
 protected:  // overridden methods
  ...
  virtual void endRun() override {         // end of run processing
...
class IMPKMEANSEXPORT KMlocalSwap : public KMlocal {
 ...
 protected:  // overridden methods
  ...
  virtual void beginRun() override {  // start of run processing
    KMlocal::beginRun();     // base class processing
    swapNo = 0;              // init number of swaps
  }
  virtual KMalg selectMethod() override {  // method = Swap
    return SWAP;
  }
  virtual void endStage() override {}        // do nothing
  virtual bool isRunDone() override {        // run is done
  ...
  virtual void tryAcceptance() override { // test acceptance
...
class IMPKMEANSEXPORT KMlocalHybrid : public KMlocal {
 ...
 protected:  // overridden methods
  ...
  virtual void beginStage() override {  // start of stage processing
    prevDist = curr.getDist();  // save previous distortion
  }
  virtual KMalg selectMethod() override {  // select method
...
#endif /* IMPKMEANS_INTERNAL_KMLOCAL_H */
