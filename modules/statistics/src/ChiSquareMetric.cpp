...
ChiSquareMetric::ChiSquareMetric(Floats data_exp, int constr_type):
 ...
 data_exp_(data_exp), constr_type_(constr_type){}

void ChiSquareMetric::add_configuration
                                     (Floats data, Floats stddev, double weight)
{
 // store weight
...
double ChiSquareMetric::do_get_weight(unsigned i) const
{
 return weight_[i];
}

unsigned ChiSquareMetric::get_number_of_items() const
{
 return datas_.size();
}

Floats ChiSquareMetric::get_data(unsigned i) const
{
 return datas_[i];
}
