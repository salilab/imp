...
namespace {
bool comp_first_smaller_than_second(const ValInd &a, const ValInd &b) {
...
void VQClustering::sample_data_point(Array1DD &p) { p = full_data_->sample(); }
...
void VQClustering::sampling(Array1DD_VEC *tracking) {
  ...
  boost::timer::progress_display show_progress(show_number_of_runs);
  ...
  for (int run_ind = 0; run_ind < par_.number_of_runs_; ++run_ind) {
    ...
    for (int step_ind = 0; step_ind < par_.number_of_steps_; step_ind++) {
      ...
      epsilon =
          par_.ei_ * exp((1. * step_ind / par_.number_of_steps_) * log_ef_ei);
      lambda =
  ...
  if (par_.eq_clusters_) {
    ...
    for (int ll = 0; ll < k_; ll++) {
      std::cout << centers_sample_eq[ll][0] << " " << centers_sample_eq[ll][1]
                << " " << centers_sample_eq[ll][2] << " " << std::endl;
    }
    std::cout << "Updates centers:" << std::endl;
    for (int ll = 0; ll < k_; ll++) {
      std::cout << "From " << centers_sample[ll][0] << " "
                << centers_sample[ll][1] << " " << centers_sample[ll][2]
                << " To " << centers_sample_eq[ll][0] << " "
                << centers_sample_eq[ll][1] << " " << centers_sample_eq[ll][2]
                << " " << std::endl;
    }
...
void VQClustering::set_assignments() {
  ...
  for (int i = 0; i < k_; i++) {
    all_cen.push_back(
        algebra::Vector3D(centers_[i][0], centers_[i][1], centers_[i][2]));
  }
