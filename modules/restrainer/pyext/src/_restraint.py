...
class _RestraintExcludedVolume(_ExcludedVolumeRestraintNode):
    ...
    def __init__(self, attributes):
        _ExcludedVolumeRestraintNode.__init__(
            self, attributes, "ExcludedVolume")
...
class _RestraintRestraint(_RestraintNode):
    ...
    def create_force(self):
        for child in self._children:
            ...
            if repr and repr.force_field == 1:
                ...
                cont = IMP.container.ListSingletonContainer(
                    bonds, "bonds_%s" %
                    id)
                ...
                m.add_restraint(IMP.container.SingletonsRestraint(bss, cont))
                cont = IMP.container.ListSingletonContainer(
                    dihedrals, "dihedrals_%s" %
                    id)
                bss = IMP.atom.DihedralSingletonScore()
                m.add_restraint(IMP.container.SingletonsRestraint(bss, cont))
                cont = IMP.container.ListSingletonContainer(
                    impropers, "impropers_%s" %
                    id)
    ...
    def create_connectivity_restraint(self, repr):
        ...
        if IMP.core.RigidBody.get_is_setup(first_particle):
            ...
            self.sc = IMP.restrainer.create_simple_connectivity_on_rigid_bodies(
                rbs, refiner)
        else:
            self.sc = IMP.restrainer.create_simple_connectivity_on_molecules(
                mhs)
        self.sc.set_k(k)
    ...
    def create_excluded_volume_restraint(self, repr):
        ...
        if IMP.core.RigidBody.get_is_setup(first_particle):
            ...
            for mh in mhs:
                rbs_tmp.append(mh.get_particle())
            rbs = IMP.core.RigidBodies(rbs_tmp)
            self.sev = IMP.restrainer.create_simple_excluded_volume_on_rigid_bodies(
                rbs)
        else:
            self.sev = IMP.restrainer.create_simple_excluded_volume_on_molecules(
                mhs)
    ...
    def create_saxs_restraint(self, repr):
        ...
        if self.profile_filename:
            ...
            if particles:
                self.saxs_restraint = IMP.saxs.Restraint(
                    particles, self.exp_profile)
    ...
    def create_distance_restraint(self, repr):
        ...
        self.imp_restraint = IMP.core.DistanceRestraint(
            h, self.child_restraints[0],
