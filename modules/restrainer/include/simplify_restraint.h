...
/** Creates ConnectivityRestraint on molecules using LowestRefinedPairScore
  ...
  */
IMPRESTRAINEREXPORT SimpleConnectivity
    create_simple_connectivity_on_molecules(const atom::Hierarchies &mhs);
...
/** Creates DiameterRestraint using HarmonicUpperBound scoring function
    ...
    See SimpleDiameter
  */
IMPRESTRAINEREXPORT SimpleDiameter
    create_simple_diameter(const kernel::Particles &ps, Float diameter);
...
*/
IMPRESTRAINEREXPORT SimpleExcludedVolume
    create_simple_excluded_volume_on_molecules(const atom::Hierarchies &mhs);
...
/** Creates EM FitRestraint.
    ...
    See SimpleEMFit
*/
IMPRESTRAINEREXPORT SimpleEMFit
    create_simple_em_fit(atom::Hierarchies const &mhs, em::DensityMap *dmap);
//! Creates EM FitRestraint.
/**
...
IMPRESTRAINEREXPORT em::DensityMap *load_em_density_map(char const *map_fn,
                                                        float spacing,
                                                        float resolution);
...
class IMPRESTRAINEREXPORT SimpleConnectivity {
  IMP_NO_SWIG(
      ...
      friend IMPRESTRAINEREXPORT SimpleConnectivity create_simple_connectivity_on_molecules(
          atom::Hierarchies const & mhs);)
 public:
   ...
   */
  void set_mean(Float mean) { harmonic_upper_bound_->set_mean(mean); }
  ...
  /** The default k is 1.
   */
  void set_k(Float k) { harmonic_upper_bound_->set_k(k); }

  base::VersionInfo get_version_info() const {
    return base::VersionInfo("restrainer", get_module_version());
  }

  void show(std::ostream &out = std::cout) const {
    out << "SimpleConnectivity(";
    if (connectivity_restraint_) connectivity_restraint_->show(out);
    out << ")";
  }

 private:
  // prevent unauthorized creation
  SimpleConnectivity(core::ConnectivityRestraint *connectivity_restraint,
...
class IMPRESTRAINEREXPORT SimpleDistance {
 ...
 public:
  core::DistanceRestraint *get_restraint() { return distance_restraint_; }
  ...
  void show(std::ostream &out = std::cout) const {
    out << "SimpleDistance(";
    if (distance_restraint_) distance_restraint_->show(out);
    out << ")";
  }

 private:
  // prevent unauthorized creation
  SimpleDistance(core::DistanceRestraint *distance_restraint,
                 core::HarmonicUpperBound *harmonic_upper_bound)
...
class IMPRESTRAINEREXPORT SimpleDiameter {
 ...
 public:
  core::DiameterRestraint *get_restraint() { return diameter_restraint_; }
  ...
  void show(std::ostream &out = std::cout) const {
    out << "SimpleDiameter(";
    if (diameter_restraint_) diameter_restraint_->show(out);
    out << ")";
  }

 private:
  // prevent unauthorized creation
  SimpleDiameter(core::DiameterRestraint *diameter_restraint,
                 core::HarmonicUpperBound *harmonic_upper_bound)
...
class IMPRESTRAINEREXPORT SimpleExcludedVolume {
 ...
 public:
  ...
  void show(std::ostream &out = std::cout) const {
    ...
    if (excluded_volume_restraint_) excluded_volume_restraint_->show(out);
    out << ")";
  }

 private:
  // prevent unauthorized creation
  SimpleExcludedVolume(core::ExcludedVolumeRestraint *excluded_volume_restraint)
...
class IMPRESTRAINEREXPORT SimpleEMFit {
  IMP_NO_SWIG(friend IMPRESTRAINEREXPORT SimpleEMFit
                  create_simple_em_fit(atom::Hierarchies const &mhs,
                                       em::DensityMap *dmap));

 public:
  em::FitRestraint *get_restraint() { return fit_restraint_; }
  ...
  void show(std::ostream &out = std::cout) const {
    out << "SimpleEMFit(";
    if (fit_restraint_) fit_restraint_->show(out);
    out << ")";
  }

 private:
  // prevent unauthorized creation
  SimpleEMFit(em::FitRestraint *fit_restraint)
