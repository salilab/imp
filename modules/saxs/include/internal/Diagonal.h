#ifndef IMP_DIAGONAL_H
#define IMP_DIAGONAL_H
...
private:
  ...
  void checkdim(int m, int n) { if (m<0 || n<0) Matrix::xerror(4,"Diagonal::checkdim"); return; }
...
public:
  ...
  Diagonal operator += (double x) { for (int i=0; i<mm_; i++) data_[i]+=x; return *this; }
  Diagonal operator -= (double x) { for (int i=0; i<mm_; i++) data_[i]-=x; return *this; }
  Diagonal operator *= (double x) { for (int i=0; i<mm_; i++) data_[i]*=x; return *this; }
  Diagonal operator /= (double x) { for (int i=0; i<mm_; i++) data_[i]/=x; return *this; }
