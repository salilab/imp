                                          ...
                                          float density)
{
  ...
  // init grid
  typedef IMP::algebra::Grid3D<IMP::Ints> Grid;
  ...
  for(unsigned int i=0; i<ps.size(); i++) {
    ...
    for (Grid::IndexIterator it= grid.indexes_begin(lb, ub);
       ...
       for(unsigned int vIndex=0; vIndex<grid[*it].size(); vIndex++) {
        ...
        float dist2 =
          algebra::get_squared_distance(coordinates[i], coordinates[mol_index]);

        if(dist2 < radius_sum1*radius_sum1) neighbours1.push_back(mol_index);
        else
          if(dist2 < radius_sum2*radius_sum2) neighbours2.push_back(mol_index);
      }
    }
    ...
    for(unsigned int s_index = 0; s_index<spoints.size(); s_index++) {
      ...
      if(!collides) { // check for intersection with neighbours2
        for(unsigned int n_index=0; n_index<neighbours2.size(); n_index++) {
