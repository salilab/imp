/**
 *  \file internal/inference_utility.h
 *  \brief Inference utilities.
 *
...
inline EdgeData get_edge_data(const Subset &s0, const Subset &s1,
  ...
  for (unsigned int i = 0; i < sft.size(); ++i) {
    SubsetFilter *sf = sft[i]->get_subset_filter(ret.union_subset, excluded);
    if (sf) {
...
inline Assignment get_sub_assignment(const Assignment &ss0, const Ints &i0) {
  Ints ret(i0.size());
...
inline bool get_are_equal(const Assignment &ss0, const Ints &i0,
                          const Assignment &ss1, const Ints &i1) {
...
IMPDOMINOEXPORT void load_union(const Subset &s0, const Subset &s1,
                                ...
                                AssignmentContainer *nd1, const EdgeData &ed,
                                size_t max, AssignmentContainer *out);
