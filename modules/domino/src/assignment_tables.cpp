...
class Counter {
 ...
 public:
  Counter(const Ints &maxs) : cur_(maxs.size(), 0), maxs_(maxs) {}
  ...
  const Ints &get_counter() const { return cur_; }
};

void brute_load_assignments(const Subset &s, ParticleStatesTable *pst,
                            const SubsetFilterTables &sft, unsigned int max,
...
void recursive_load_assignments(const Subset &s, ParticleStatesTable *pst,
                                const SubsetFilterTables &sft, int max,
                                SimpleAssignmentsTable *sat,
                                AssignmentContainer *pac) {
  if (s.size() <= 2) {
    ...
    recursive_load_assignments(s0, pst, sft, max, sat, pac0);
    recursive_load_assignments(s1, pst, sft, max, sat, pac1);
    IMP_LOG_TERSE("Merging " << s0 << "(" << pac0->get_number_of_assignments()
                             << ") and " << s1 << "("
    ...
    for (unsigned int i = 0; i < pac0->get_number_of_assignments(); ++i) {
      ...
      for (unsigned int j = 0; j < a1.size(); ++j) {
        Assignment merged =
            internal::get_merged_assignment(s, a0, ui0, a1[j], ui1);
        bool ok = true;
...
void SimpleAssignmentsTable::load_assignments(const Subset &s,
                                              AssignmentContainer *pac) const {
  set_was_used(true);
  IMP_OBJECT_LOG;
...
ParticlesTemp initialize_order(const Subset &s, const SubsetFilterTables &sft) {
  IMP_FUNCTION_LOG;
  Ints order;
  Ints remaining;
...
Assignment get_next_assignment(const ParticlesTemp &s, const Subsets &subsets,
  ...
  while (true) {
    ...
    for (unsigned int i = 0; i < filters.back().size(); ++i) {
      if (!filters.back()[i]->get_is_ok(cura)) {
        ...
        unsigned int next = filters.back()[i]->get_next_state(pos, cura);
        increment = std::max<unsigned int>(next - cur.back(), 1);
        IMP_IF_CHECK(USAGE_AND_INTERNAL) {
          int maxnext = std::min<unsigned int>(next, maxs[orders.back()[pos]]);
          ...
          for (unsigned int j = cura[pos];
            ...
            IMP_INTERNAL_CHECK(
                !filters.back()[i]->get_is_ok(curat),
...
BranchAndBoundAssignmentsTable::BranchAndBoundAssignmentsTable(
    ParticleStatesTable *pst, const SubsetFilterTables &sft, unsigned int max)
    : pst_(pst), sft_(sft), max_(max) {
  IMP_OBJECT_LOG;
...
void BranchAndBoundAssignmentsTable::load_assignments(
    const Subset &s, AssignmentContainer *pac) const {
  set_was_used(true);
  IMP_OBJECT_LOG;
  ...
  do {
    Assignment cura;
    if (spt.size() > 1) {
      cura = get_next_assignment(spt, subsets, orders, cur, maxs, filters);
    } else {
      cura = get_next_assignment_base(cur, maxs, filters);
    }
    if (cura.size() > 0) {
  ...
  IMP_IF_CHECK(USAGE_AND_INTERNAL) {
    ...
    for (unsigned int i = 0; i < s.size(); ++i) {
      ...
      IMP_INTERNAL_CHECK(n > 0 && n < 1000000, "Out of range num states" << n);
      space *= n;
      if (space > 10000) {
        break;
      }
    ...
    if (space < 10000) {
      ...
      if (cpac->get_number_of_assignments() !=
        ...
        Assignments diff;
        std::set_symmetric_difference(a0.begin(), a0.end(), a1.begin(),
                                      a1.end(), std::back_inserter(diff));
        IMP_LOG(WARNING, a0 << "\n vs \n" << a1 << "\n diff\n" << diff);
        IMP_INTERNAL_CHECK(
            0, "Numbers don't match "
                   << cpac->get_number_of_assignments() << " vs "
                   << pac->get_number_of_assignments() << " for " << s << "\n");
      }
...
void ListAssignmentsTable::load_assignments(const Subset &s,
