...
void WriteHDF5AssignmentContainer::flush() {
  ...
  IMP_LOG_VERBOSE("Flushing cache of size " << num_items << " to disk"
                                            << std::endl);
  nsize[0] += num_items;
  ds_.set_size(nsize);
...
void WriteHDF5AssignmentContainer::add_assignment(const Assignment &a) {
  IMP_USAGE_CHECK(a.size() == order_.size(),
                  "Sizes don't match: " << a.size() << " vs " << order_.size());
...
void WriteAssignmentContainer::add_assignment(const Assignment &a) {
  ...
  ++number_;
  IMP_LOG_VERBOSE("Added " << a << " cache is now " << cache_ << std::endl);
  if (cache_.size() > max_cache_) flush();
}

/// Capped writer
CappedAssignmentContainer::CappedAssignmentContainer(AssignmentContainer *c,
                                                     int max, std::string name)
    : P(name), contained_(c), max_(max) {}
...
RangeViewAssignmentContainer::RangeViewAssignmentContainer(
    AssignmentContainer *inner, unsigned int begin, unsigned int end)
...
HeapAssignmentContainer::HeapAssignmentContainer(Subset subset, unsigned int k,
                                                 RestraintCache *rssf,
                                                 std::string name)
...
void HeapAssignmentContainer::add_assignment(const Assignment &a) {
  ...
  while (d_.size() > k_) {
    std::pop_heap(d_.begin(), d_.end(), GreaterSecond());
    d_.pop_back();
  }
