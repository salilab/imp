...
namespace {
void load_node_data(const SubsetGraph &sg, const AssignmentsTable *sst,
...
void load_edge_data(const SubsetGraph &sg, const SubsetFilterTables &sft,
  ...
  for (unsigned int i = 0; i < data.size(); ++i) {
    ...
    for (std::pair<NeighborIterator, NeighborIterator> be =
      ...
      if (*be.first < i) {
        data[i].push_back(
            get_edge_data(subset_map[i], subset_map[*be.first], sft));
      }
    }
  }
}

bool loopy_filter_edge(const Subset &s0, const Subset &s1, Assignments &nd0,
                       const Assignments &nd1, const EdgeData &ed) {
  ...
  for (unsigned int i = 0; i < nd0.size(); ++i) {
    ...
    for (unsigned int j = 0; j < nd1.size(); ++j) {
      if (get_are_equal(nd0[i], ii0, nd1[j], ii1)) {
        Assignment ss =
            get_merged_assignment(ed.union_subset, nd0[i], ui0, nd1[j], ui1);
        bool filter_ok = true;
  ...
  for (int i = to_erase.size() - 1; i >= 0; --i) {
    IMP_LOG_VERBOSE("Erasing " << nd0[to_erase[i]] << " from " << s0
                               << std::endl);
...
bool filter_pass(const SubsetGraph &sg, const base::Vector<EdgeDatas> &eds,
                 base::Vector<Assignments> &data) {
  bool changed = false;
  boost::property_map<SubsetGraph, boost::vertex_name_t>::const_type
      subset_map = boost::get(boost::vertex_name, sg);
  for (unsigned int i = 0; i < data.size(); ++i) {
    ...
    for (std::pair<NeighborIterator, NeighborIterator> be =
         ...
         be.first != be.second; ++be.first) {
      if (*be.first < i) {
        changed |= loopy_filter_edge(subset_map[i], subset_map[*be.first],
                                     data[i], data[*be.first], eds[i][j]);
        changed |= loopy_filter_edge(subset_map[*be.first], subset_map[i],
                                     data[*be.first], data[i], eds[i][j]);
        ++j;
      }
    }
  }
...
void write(boost::property_map<SubsetGraph, boost::vertex_name_t>::const_type
               subset_map,
           const base::Vector<Assignments> &nds, std::string name) {
...
Assignments loopy_get_best_conformations(const SubsetGraph &sg, const Subset &,
  ...
  boost::property_map<SubsetGraph, boost::vertex_name_t>::const_type
      subset_map = boost::get(boost::vertex_name, sg);
  IMP_IF_LOG(TERSE) { write(subset_map, nds, "Before filtering"); }
  while (filter_pass(sg, eds, nds)) {
    IMP_IF_LOG(TERSE) { write(subset_map, nds, "Now"); }
  }
  IMP_IF_LOG(TERSE) { write(subset_map, nds, "After filtering"); }
