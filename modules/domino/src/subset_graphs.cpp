...
void triangulate(InteractionGraph &ig) {
  typedef std::pair<InteractionGraphTraits::adjacency_iterator,
                    InteractionGraphTraits::adjacency_iterator> AdjacencyRange;
  typedef std::pair<InteractionGraphTraits::vertex_iterator,
                    InteractionGraphTraits::vertex_iterator> VertexRange;
  ...
  while (boost::num_vertices(mig) > 0) {
    ...
    for (VertexRange be = boost::vertices(mig); be.first != be.second;
         ++be.first) {
...
SubsetGraph get_minimum_spanning_tree(const CliqueGraph &cg) {
  ...
  for (unsigned int i = 0; i < mst.size(); ++i) {
    boost::add_edge(boost::source(mst[i], cg), boost::target(mst[i], cg), jt);
  }
  // find disjoint sets and connect them arbitrarily
  {
    ...
    for (std::pair<SubsetGraphTraits::edge_iterator,
      ...
      uf.union_set(boost::source(*be.first, jt), boost::target(*be.first, jt));
    }
...
bool get_has_edge(InteractionGraph &graph, InteractionGraphVertex va,
  ...
  for (unsigned int i = 0; i < pt.size(); ++i) {
    ...
    for (unsigned int k = 0; k < i; ++k) {
      ...
      if (vj != vk && !get_has_edge(g, vj, vk)) {
        IMP_LOG_VERBOSE("Adding edge between \""
                        << ps[vj]->get_name() << "\" and \""
                        << ps[vk]->get_name() << "\" due to \""
                        << blame->get_name() << "\"" << std::endl);
...
namespace {
int create_set_node(
    const Subset &s, MergeTree &merge_tree,
    boost::property_map<MergeTree, boost::vertex_name_t>::type &mt_sets) {
  int vi = boost::add_vertex(merge_tree);
  mt_sets[vi] = s;
  return vi;
}
int create_merge_tree_internal(
    const SubsetGraph &junction_tree,
    const boost::property_map<SubsetGraph, boost::vertex_name_t>::const_type &
        jt_sets,
    int cur_jt, int last_jt,  // avoid parent
  ...
  for (std::pair<SubsetGraphTraits::out_edge_iterator,
                 SubsetGraphTraits::out_edge_iterator> ebe =
           boost::out_edges(cur_jt, junction_tree);
       ebe.first != ebe.second; ++ebe.first) {
    int target = boost::target(*ebe.first, junction_tree);
    if (target == last_jt) continue;
    int child = create_merge_tree_internal(junction_tree, jt_sets, target,
                                           cur_jt, merge_tree, mt_sets);
...
MergeTree get_merge_tree(const SubsetGraph &junction_tree /*, int start*/) {
  ...
  unsigned int root = create_merge_tree_internal(
      junction_tree, boost::get(boost::vertex_name, junction_tree), start, -1,
      merge_tree, mt_sets);
...
bool get_is_merge_tree(
    ...
    const boost::property_map<MergeTree, boost::vertex_name_t>::const_type &
        mt_sets,
    bool verbose, int cur, int parent) {
...
bool get_is_merge_tree(const MergeTree &tree, Subset all, bool verbose) {
  boost::property_map<MergeTree, boost::vertex_name_t>::const_type mt_sets =
      boost::get(boost::vertex_name, tree);
  int nv = boost::num_vertices(tree);
  Subset maybeall = mt_sets[nv - 1];
  if (maybeall != all) {
    if (verbose) {
      IMP_WARN("Root does not contain all particles: " << all << " vs "
                                                       << maybeall);
      return false;
    }
  }
  return get_is_merge_tree(tree, mt_sets, verbose, nv - 1, -1);
}
...
base::Vector<SSGED> get_independent_edge_set(const StableSubsetGraph &sg) {
  ...
  for (unsigned int i = 0; i < edges.size(); ++i) {
    ...
    if (seen.find(source) == seen.end() && seen.find(target) == seen.end()) {
...
SSGVD merge_edge(SSGED e, StableSubsetGraph &jt) {
  ...
  IMP_INTERNAL_CHECK(source != target, "Can't handle self edges");
  boost::property_map<StableSubsetGraph, boost::vertex_name_t>::type jt_sets =
