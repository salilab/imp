...
#define IMP_DEFINE_INTERNAL_LINKERS(Name, name, args, cargs, create_args,      \
                                    create_cargs)                              \
  namespace {                                                                  \
  Name##SaveLink* get_##name##_save_link args {                                \
    ...
    if (!fh.get_has_associated_data(index)) {                                  \
      ...
      IMP::rmf::SaveLinkAssociationType psl = new Name##SaveLink create_args;  \
  ...
  Name##LoadLink* get_##name##_load_link cargs {                               \
    ...
    if (!fh.get_has_associated_data(index)) {                                  \
      RMF::SetCurrentFrame scf(fh, RMF::ALL_FRAMES);                           \
      IMP::rmf::LoadLinkAssociationType psl = new Name##LoadLink create_cargs; \
      IMP::rmf::set_linker(fh, index, psl);                                    \
    }                                                                          \
 ...
 */
#define IMP_DEFINE_LINKERS(Name, name, names, InType, InTypes, OutType,      \
                           OutTypes, args, cargs, create_args, create_cargs, \
                           create_cargs_from)                                \
  IMP_DEFINE_INTERNAL_LINKERS(Name, name, args, cargs, create_args,          \
                              create_cargs);                                 \
  void add_##names(RMF::FileHandle fh, const OutTypes& hs) {                 \
    if (hs.empty()) return;                                                  \
    Name##SaveLink* hsl = get_##name##_save_link(fh);                        \
    RMF::SetCurrentFrame scf(fh, RMF::ALL_FRAMES);                           \
    hsl->add(fh.get_root_node(), hs);                                        \
  }                                                                          \
  void add_##names(RMF::NodeHandle fh, const OutTypes& hs) {                 \
    if (hs.empty()) return;                                                  \
  ...
  void link_##names(RMF::FileConstHandle fh, const InTypes& hs) {            \
    if (hs.empty()) return;                                                  \
