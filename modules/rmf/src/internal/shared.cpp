...
namespace rmf {
  namespace internal {
    ...
    SharedData::SharedData(HDF5Group g, bool create):
      file_(g),
      names_(file_.get_child_data_set<StringTraits>
             (get_node_name_data_set_name(),
              1)),
      node_data_(file_.get_child_data_set<IndexTraits>
                 (get_node_data_data_set_name(),
                  2)),
      bond_data_(file_.get_child_data_set<IndexTraits>
                 (get_bond_data_data_set_name(),
                  2)),
      frames_hint_(0),
      last_node_(-1), last_vi_(-1)
    {
      ...
      for ( int i=0; i< dim[0]; ++i) {
        if (IndexTraits::
            get_is_null_value(node_data_.get_value(make_index(i,
                                                              0)))) {
    ...
    int SharedData::add_node(std::string name, unsigned int type) {
      ...
      node_data_.set_value(make_index(ret, CHILD),
                           IndexTraits::get_null_value());
      node_data_.set_value(make_index(ret, SIBLING),
                           IndexTraits::get_null_value());
      return ret;
    }
