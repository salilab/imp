  ...
  if (fhc.get_type() == RMF::BOND && children.size() == 2) {
    ...
    if (map.find(bd0) != map.end() && map.find(bd1) != map.end()) {
      ...
      atom::create_bond(get_bonded(map.find(bd0)->second),
                        get_bonded(map.find(bd1)->second), atom::Bond::SINGLE);
    }
  } else {
...
void HierarchyLoadLink::do_load_node(RMF::NodeConstHandle nh, kernel::Particle *o) {
  if (rigid_factory_.get_is(nh)) {
    ...
    algebra::Transformation3D tr(
        algebra::Rotation3D(algebra::Vector4D(orient.begin(), orient.end())),
        v);
    ...
    RMF::Floats cs = intermediate_particle_factory_.get(nh).get_coordinates();
    algebra::Vector3D v(cs.begin(), cs.end());
...
bool HierarchyLoadLink::setup_particle(kernel::Particle *root, RMF::NodeConstHandle nh,
  ...
  if (nh.get_has_value(rigid_body_key_) && !rbp) {
    ...
    RMF::Floats cs = intermediate_particle_factory_.get(nh).get_coordinates();
    algebra::Vector3D vv(cs.begin(), cs.end());
...
void HierarchyLoadLink::do_add_link_recursive(kernel::Particle *root, kernel::Particle *o,
  ...
  for (unsigned int i = 0; i < ch.size(); ++i) {
    if (ch[i].get_type() == RMF::REPRESENTATION) {
      do_add_link_recursive(root, atom::Hierarchy(o).get_child(cur), ch[i]);
      ++cur;
    }
  }
}
...
void HierarchySaveLink::do_save_node(kernel::Particle *p, RMF::NodeHandle n) {
  if (core::RigidBody::get_is_setup(p)) {
    if (atom::Hierarchy(p).get_number_of_children() == 0) {
      ...
      p.set_translation(
          RMF::Floats(t.coordinates_begin(), t.coordinates_end()));
    } else {
      core::RigidBody bd(p);
