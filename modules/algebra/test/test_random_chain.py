...
class Tests(IMP.test.TestCase):
    ...
    def test_random_chain(self):
        ...
        for i in range(0, len(chain) - 1):
            self.assertAlmostEqual(
                IMP.algebra.get_distance(chain[i], chain[i + 1]),
                2 * r, delta=.1 * 2 * r)
            # print ".cylinder " + str(chain[i]
        for i in range(0, len(chain)):
            for j in range(0, i - 1):
                self.assertGreater(
                    IMP.algebra.get_distance(chain[i], chain[j]),
                    .9 * 2 * r)
    ...
    def test_random_chain_obst(self):
        ...
        for i in range(0, 100):
            c = IMP.algebra.get_random_vector_in(
                IMP.algebra.Sphere3D(IMP.algebra.Vector3D(10, 10, 10),
                                     3.0))
            cr = r / 2.0
