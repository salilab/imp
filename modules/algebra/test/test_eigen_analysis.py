...
class Tests(IMP.test.TestCase):
    ...
    def test_pca(self):
        ...
        self.assertAlmostEqual(
            ed.get_principal_component(0)[0],
            0.677873,
            delta=.01)
        self.assertAlmostEqual(
            ed.get_principal_component(0)[1],
            0.735179,
            delta=.01)
        self.assertAlmostEqual(
    ...
    def test_pca_from_random_points(self):
        ...
        ed = IMP.algebra.get_principal_components(vs)
        self.assertGreater(
            ed.get_principal_value(0),
            ed.get_principal_value(1))
        self.assertGreater(
            ed.get_principal_value(1),
            ed.get_principal_value(2))
        self.assertAlmostEqual(
            IMP.algebra.get_distance(vs_mean,
                                     ed.get_centroid()),
            0.,
            delta=.1)
    ...
    def test_that_pca_is_invariant_to_rotation(self):
        ...
        rand_trans = IMP.algebra.Transformation3D(
            rand_rot, IMP.algebra.Vector3D(0., 0., 0.))
        for i in range(50):
            vs[i] = rand_trans.get_transformed(vs[i])
        ed2 = IMP.algebra.get_principal_components(vs)
        self.assertAlmostEqual(
            ed1.get_principal_value(0),
            ed2.get_principal_value(0),
            delta=0.2)
        self.assertAlmostEqual(
            ed1.get_principal_value(1),
            ed2.get_principal_value(1),
            delta=0.2)
        self.assertAlmostEqual(
            ed1.get_principal_value(2),
