...
class Tests(IMP.test.TestCase):
    def test_access(self):
        ...
        self.assertAlmostEqual(IMP.algebra.get_squared_distance(cent, exp_cent),
                               0.0, delta=1e-5)
        new_cent = IMP.algebra.Vector3D(8,9,10)
        pca.set_centroid(new_cent)
        cent = pca.get_centroid()
        self.assertAlmostEqual(IMP.algebra.get_squared_distance(cent, new_cent),
                               0.0, delta=1e-5)
    ...
    def test_get_pca(self):
        ...
        cent = pca.get_centroid()
        self.assertAlmostEqual(IMP.algebra.get_squared_distance(
                                        cent, IMP.algebra.Vector3D(0.5,0,0.5)),
                               0.0, delta=1e-5)
