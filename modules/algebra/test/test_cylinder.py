...
class CylinderTests(IMP.test.TestCase):
    ...
    def test_cylinder_construction(self):
        ...
        self.assertEqual((cyl.get_main_direction()-direction).get_magnitude() < 0.01,True)
        self.assertEqual(cyl.get_radius(),5.0)
        self.assertEqual(cyl.get_height(),8.0)

        self.assertAlmostEqual(cyl.get_area(),2*math.pi*5.0*8.0+2*math.pi*25.0,
                               places=1)
    ...
    def test_uniform_cover(self):
        ...
        cyl = IMP.algebra.Cylinder3D(IMP.algebra.Vector3D(0.0,0.0,-4.0),
                                   IMP.algebra.Vector3D(0.0,0.0,4.0),
                                   5.0);
        points = IMP.algebra.uniform_cover(cyl,8,8)
    ...
    def test_uniform_cover_center_not_at_000(self):
