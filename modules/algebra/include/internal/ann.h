...
struct ANNData {
  ...
  template <class It> unsigned int extract_dimension(It b, It e) const {
    ...
    for (It c = b; c != e; ++c) {
      ...
      if (ret == 0) {
        ...
        IMP_USAGE_CHECK(v.get_dimension() == ret, "Dimensions don't match");
      }
    }
    return ret;
  }
  ...
  template <class It>
  ANNData(It b, It e)
      : dimension_(extract_dimension(b, e)),
  ...
  void fill_nearest_neighbors(const G &g, double distance, double eps,
    ...
    if (k >= guess) {
      ...
      boost::scoped_array<ANNdist> dists(new ANNdist[k]);
      tree_.annkFRSearch(pt, square(fix_distance(distance, eps)), k, &ret[0],
                         dists.get(), eps);
    }
  }
  ...
  unsigned int get_number_of_points() const { return tree_.nPoints(); }
  ~ANNData() {
