...
Triangle3D::Triangle3D(const Vector3D &p1, const Vector3D &p2,
                       const Vector3D &p3) {
  p_[0] = p1;
  p_[1] = p2;
  p_[2] = p3;
...
Triangle3D get_largest_triangle(const Vector3Ds &points) {
  ...
  max_dist = 0;
  for (unsigned int i = 0; i < points.size(); i++) {
    double dist = algebra::get_squared_distance(seg, points[i]);
    if (dist > max_dist) {
      max_dist = dist;
      triple[2] = points[i];
    }
...
ReferenceFrame3D get_reference_frame_of_triangle(Triangle3D t) {
  ...
  Rotation3D rot = get_rotation_from_matrix(x[0], x[1], x[2], y[0], y[1], y[2],
                                            z[0], z[1], z[2]);
