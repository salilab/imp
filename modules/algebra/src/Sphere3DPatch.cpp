...
SpherePatch3D::SpherePatch3D(const Sphere3D &sph, const Plane3D &crossing_plane)
...
Vector3D SpherePatch3D::get_boundary_point() const {
  ...
  Vector3D v =
      get_orthogonal_vector(crossing_plane_.get_normal()).get_unit_vector();
  ...
  double c =
      -2. * cen * p + cen * cen + p * p - sph_.get_radius() * sph_.get_radius();
  double b = 2. * (p * v + cen * v);
  double a = v * v;
  double f = (-b + std::sqrt(b * b - 4 * a * c)) / (2 * a);
  IMP_USAGE_CHECK(!IMP::isnan(f),
