...
void handle_atom(ConnollySurfacePoints& surface_points,
...
SurfaceInfo generate_contact_surface(ConnollySurfacePoints& surface_points,
                              ...
                              std::vector<ProbePoint> &beforept)
{
  ...
  for (unsigned i = 0; i < CO.size(); ++i) {
    ...
    if (cube.get_neighbors(i, CO, rp, IAT, rtype, itnl)) {
      ...
      handle_atom(surface_points, i, d, itnl, CO, rp, srs, up, yon_probes, circle,
  ...
  for (std::vector<ProbePoint>::const_iterator pit = beforept.begin();
    ...
    for (std::vector<SurfacePoint>::const_iterator sit = pit->points.begin();
      ...
      if (!point_eaten) {
        ...
        ConnollySurfacePoint sp(sit->n1, sit->n2, sit->n3, sit->s, sit->area, outvec);
        surface_points.push_back(sp);
      } else {
        if (pit->ishape == 2) {
                                       ...
                                       float rp, float radmax)
{
  ...
  check_eaten_points(surface_points, yon_probes, beforept, rp, victims, surface);
  return surface;
}

void msdots(ConnollySurfacePoints& surface_points,
            float d, float rp,
            const std::vector<float> &rtype,
            ...
            const std::vector<int> &IAT)
{
  ...
  SurfaceInfo reentrant_surface =
      generate_reentrant_surface(surface_points, yon_probes, beforept, rp, radmax);
