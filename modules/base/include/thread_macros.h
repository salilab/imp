...
#define IMP_TASK(privatev, action, name)                           \
  if (IMP::base::get_number_of_threads() > 1) {                    \
    IMP_OMP_PRAGMA(task default(none)                              \
                   firstprivate privatev if (omp_in_parallel())) { \
...
#define IMP_TASK_SHARED(privatev, sharedv, action, name)                     \
  if (IMP::base::get_number_of_threads() > 1) {                              \
    IMP_OMP_PRAGMA(task default(                                             \
        none) firstprivate privatev shared sharedv if (omp_in_parallel())) { \
      IMP::base::CreateLogContext task_context(name);                        \
      action;                                                                \
    }                                                                        \
  } else {                                                                   \
...
#define IMP_THREADS(variables, action)                            \
  if (IMP::base::get_number_of_threads() > 1) {                   \
    IMP_OMP_PRAGMA(parallel shared variables num_threads(         \
