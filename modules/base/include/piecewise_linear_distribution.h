...
IMPBASE_BEGIN_NAMESPACE /** Draw random numbers from a distribution defined as a
                            piecewise
...
class piecewise_linear_distribution {
  ...
  double get_divider(int divider) const { return dividers_[divider]; }
  double get_accum(int divider) const { return accum_[divider]; }
  double get_slope(int divider) const {
    return (weights_[divider + 1] - weights_[divider]) /
           (dividers_[divider + 1] - dividers_[divider]);
  }
  double get_weight(int divider) const { return weights_[divider]; }
  ...
  */
  ...
  piecewise_linear_distribution(LIt locations_begin, LIt locations_end,
                                WIt weights_begin)
      : dividers_(locations_begin, locations_end),
