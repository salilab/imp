...
/*
    ...
    from Decorator to kernel::Particle* (so their swig type info is passed
    around
...
struct ConvertSequence<base::Array<D, T, TS>, ConvertT> {
  ...
  static base::Array<D, T, TS> get_cpp_object(PyObject* o, SwigData st,
    ...
    if (PySequence_Size(o) != D) {
      IMP_THROW("Expected tuple of size " << D << " but got one of size "
                                          << PySequence_Size(o),
                ValueException);
    }
...
struct ConvertSequence<
    base::ConstVector<T, TS>,
    ConvertT> : public ConvertVectorBase<base::ConstVector<T, TS>, ConvertT> {
