...
class Tests(IMP.test.TestCase):
    ...
    def test_graph_scoring(self):
        ...
        graph, graph_scores = spatiotemporal.create_DAG(state_dict, input_dir=input, out_cdf=False,dag_heatmap=False,out_labeled_pdf=False,draw_dag=False)
    ...
    def test_temporal_scoring(self):
        ...
        with IMP.test.temporary_directory() as tmpdir:
            input = os.path.join(tmpdir, 'data')
        shutil.copytree(self.get_input_file_name('data'), input)
        graph, graph_scores = spatiotemporal.create_DAG(state_dict, input_dir=input, out_cdf=False,dag_heatmap=False,out_labeled_pdf=False,spatio_temporal_rule=True,expected_subcomplexes=expected_subcomplexes,draw_dag=False)
        # Check the overall score for the first trajectory
        for trajectory in graph_scores:
            if trajectory[0][0].get_label() == '1' and trajectory[0][1].get_label() == '1' and trajectory[0][2].get_label() == '1':
                self.assertAlmostEqual(trajectory[1], 2.0, delta=1e-4)
    ...
    def test_composition_scoring(self):
        ...
        with IMP.test.temporary_directory() as tmpdir:
            input = os.path.join(tmpdir, 'data')
        shutil.copytree(self.get_input_file_name('data'), input)
        graph, graph_scores = spatiotemporal.create_DAG(state_dict, input_dir=input, out_cdf=False, dag_heatmap=False,out_labeled_pdf=False, score_comp=True,exp_comp_map=exp_comp_map,draw_dag=False,spatio_temporal_rule=False,expected_subcomplexes=expected_subcomplexes)
    ...
    def test_writing_output(self):
        ...
        spatiotemporal.create_DAG(state_dict, input_dir=input,output_dir=output, dag_heatmap=False,out_labeled_pdf=True,out_cdf=True,out_pdf=True,draw_dag=False)
        # Read in input files
    ...
    def test_writing_dag(self):
        ...
        try:
            ...
            print('graphviz not available, will not test drawing graph')
            skip=1
        try:
            from matplotlib import cm
            from matplotlib import colors as clr
        except ImportError:
            print('matplotlib not available, will not test drawing graph')
            skip=1
        if skip==1:
            pass
        else:
            graph, graph_scores = spatiotemporal.create_DAG(state_dict, input_dir=input,output_dir=output, draw_dag=True,out_labeled_pdf=False,out_cdf=False,out_pdf=False)
