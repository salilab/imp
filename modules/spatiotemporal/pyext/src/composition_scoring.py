...
def get_state(subcomplex_components,prot):
    ...
    @param subcomplex_components: subcomplexes or components in a given node, which can be accessed by graphNode.get_subcomplex_components()
    @param prot: string, protein or subcomplex we are interested in finding
    @return state, int, number of times the protein or subcomplex appears in subcomplex_components
    """
    state=0
...
def composition_likelihood_function(mean, std, prots, node):
    ...
    @param mean: dictionary of dictionaries where the first key is the protein, the second key is the time, and the expected mean copy number from experiment is returned.
    @param std: dictionary of dictionaries where the first key is the protein, the second key is the time, and the expected standard deviation of protein copy number from experiment is returned.
    @param prots: list of proteins or subcomplexes which will be scored according to this likelihood function
    @param node: the graphNode object for which the likelihood will be calculated.
    @return w: float, the weight of the graphNode according to the composition likelihood function.
    """
    # get time
    t=node.get_time()
    w=0
    for prot in prots:
        ...
        if std[prot][t]>0:
            ...
            warnings.warn('WARNING!!! Standard deviation of protein '+prot+' 0 or less at time '+t+'. May lead to illogical results.')
...
def calc_likelihood(exp_comp_map,nodes):
    ...
    @param nodes: list of graphNode objects, which have been already been initiated with static scores
    @return nodes: editted list of graphNode objects, which now have static and composition scores
    """
    ...
    for prot in prots:
        ...
        if os.path.exists(exp_comp_map[prot]):
            ...
            raise Exception("Error!!! Check exp_comp_map. Unable to find composition file: " + exp_comp_map[prot] + '\nClosing...')
