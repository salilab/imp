...
void write_marker(std::ostream &out, const algebra::Vector3D &p, Color c,
                  double r) {
  out << "mark=s.place_marker((" << commas_io(p) << "), (" << commas_io(c)
      << ")";
  if (r > 0) {
    out << ", " << r;
  } else {
...
bool ChimeraWriter::handle_sphere(SphereGeometry *g, Color color,
                                  std::string name) {
  cleanup(name, true);
  write_marker(get_stream(), g->get_geometry().get_center(), color,
               g->get_geometry().get_radius());
  return true;
}
bool ChimeraWriter::handle_cylinder(CylinderGeometry *g, Color color,
                                    std::string name) {
  cleanup(name, true);
...
bool ChimeraWriter::handle_point(PointGeometry *g, Color color,
                                 std::string name) {
  cleanup(name, true);
  write_marker(get_stream(), g->get_geometry(), color, 0);
  return true;
}
...
bool ChimeraWriter::handle_triangle(TriangleGeometry *g, Color color,
                                    std::string name) {
  cleanup(name, false, true);
  get_stream() << "v=[";
...
void ChimeraWriter::add_python_code(std::string code) { get_stream() << code; }
