  ...
  IMP_IF_CHECK(base::USAGE_AND_INTERNAL) {
    ...
    IMP_INTERNAL_CHECK(
        ...
        "There are too many particles in the nbl. Currently "
            ...
            << c_->get_indexes().size() * (c_->get_indexes().size() - 1) / 2);
    IMP::base::set<ParticleIndexPair> existings(cur.begin(), cur.end());
      ...
      for (unsigned int i = 0; i < get_number_of_pair_filters(); ++i) {
        ...
        IMP_INTERNAL_CHECK(
            !get_pair_filter(i)->get_value_index(
                get_model(), ParticleIndexPair((cur[j])[0], (cur[j])[1])),
    ...
    IMP_INTERNAL_CHECK(
        ...
        "Not all particle pairs in list are unique: "
            ...
            << " vs " << ParticleIndexPairs(existings.begin(), existings.end())
            << std::endl);
    ...
    ParticlePairsTemp found = cpf_->get_close_pairs(all);
    IMP_LOG_TERSE("In check found " << found << std::endl);
    for (unsigned int i = 0; i < found.size(); ++i) {
      ParticleIndexPair pi(found[i][0]->get_index(), found[i][1]->get_index());
      ParticleIndexPair pii(found[i][1]->get_index(), found[i][0]->get_index());
      IMP_INTERNAL_CHECK(
