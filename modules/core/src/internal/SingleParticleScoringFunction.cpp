...
SingleParticleScoringFunction::SingleParticleScoringFunction(
    ParticleIndex pi, const RestraintsTemp &all, std::string name)
...
namespace {
std::pair<Ints, Restraints> get_my_restraints(
    Particle *p, const base::map<Restraint *, int> &all, Model *m) {
  RestraintsTemp cr =
      get_dependent_restraints(p, ParticlesTemp(), m->get_dependency_graph(),
  ...
  for (unsigned int j = 0; j < cr.size(); ++j) {
    if (all.find(cr[j]) != all.end()) {
      ...
      IMP_INTERNAL_CHECK(
          std::find(mr.begin(), mr.end(), cr[j]) == mr.end(),
          "Found duplicate restraint " << Showable(cr[j]) << " in list " << cr);
      mr.push_back(cr[j]);
    }
  }
  IMP_LOG_TERSE("Particle " << Showable(p) << " has restraints " << mr
                            << std::endl);
...
void SingleParticleScoringFunction::do_update_dependencies() {
  IMP_OBJECT_LOG;
  ...
  boost::tie(indexes_, mr) =
      get_my_restraints(get_model()->get_particle(pi_), mp, get_model());
  IMP_LOG_TERSE("Found " << mr << " for particle "
                         << Showable(get_model()->get_particle(pi_))
                         << std::endl);
...
ScoreStatesTemp
SingleParticleScoringFunction::get_required_score_states() const {
  IMP_OBJECT_LOG;
  ScoreStatesTemp from_restraints =
      IMP::internal::RestraintsScoringFunction::get_required_score_states();
  IMP_LOG_TERSE("Score states from restraints are "
                << from_restraints << "("
  ...
  IMP_LOG_TERSE("Score states from dummy are "
                << from_dummy << "(" << dummy_restraints_ << ")" << std::endl);
  ScoreStatesTemp deps = IMP::get_dependent_score_states(
      get_model()->get_particle(pi_), ModelObjectsTemp(),
  ...
  ScoreStatesTemp ret;
  std::set_intersection(allin.begin(), allin.end(), deps.begin(), deps.end(),
                        std::back_inserter(ret));
  IMP_LOG_TERSE("Particle " << Showable(get_model()->get_particle(pi_))
                            << " will update " << get_update_order(ret)
                            << std::endl);
  return get_update_order(ret);
}

void SingleParticleScoringFunction::add_dummy_restraint(Restraint *r) {
  IMP_USAGE_CHECK(r, "nullptr dummy restraint.");
  get_model()->clear_caches();
  dummy_restraints_.push_back(r);
}

void SingleParticleScoringFunction::clear_dummy_restraints() {
