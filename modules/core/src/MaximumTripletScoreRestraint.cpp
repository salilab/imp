/**
 *  \file MaximumTripletScoreRestraint.cpp
...
#include "IMP/core/MaximumTripletScoreRestraint.h"
...
namespace {
  typedef internal::MinimalSet<double,
    TripletContainer::ParticleTripletIterator, std::greater<Float> > MS;
...
double MaximumTripletScoreRestraint
...
void MaximumTripletScoreRestraint::show(std::ostream &out) const {
  out << "MaximumTripletScoreRestraint over ";
  c_->show(out);
  out << " using function ";
  f_->show(out);
  out << std::endl;
...
ParticlesList MaximumTripletScoreRestraint::get_interacting_particles() const
{
  MS bestn= find_minimal_set(c_->particle_triplets_begin(),
                             c_->particle_triplets_end(), f_.get(), n_);
  ParticlesList ret;
...
ParticlesTemp MaximumTripletScoreRestraint::get_input_particles() const
{
