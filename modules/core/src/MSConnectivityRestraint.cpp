...
bool MSConnectivityRestraint::ExperimentalTree::is_consistent(
  ...
  for (unsigned int i = 0; i < node->get_number_of_parents(); ++i) {
    ...
    if (child_total >= parent_total) return false;
  }
  return true;
}
...
void MSConnectivityRestraint::ParticleMatrix::create_distance_matrix(
  ...
  for (unsigned int r = 0; r < n; ++r) {
    ...
    for (unsigned int c = r + 1; c < n; ++c) {
      double d = my_evaluate(ps, particles_[r].get_particle(),
                             particles_[c].get_particle(), nullptr);
...
class Tuples {
 public:
  ...
  Ints const &get_tuple() const { return current_tuple_; }
 ...
 public:
  ...
  bool empty() const {
    for (unsigned int i = 0; i < tuples_.size(); ++i)
      if (!tuples_[i].empty()) return false;
    return true;
  }
...
bool Assignment::next() {
  for (unsigned int i = 0; i < tuples_.size(); ++i) {
    if (tuples_[i].empty()) continue;
    if (tuples_[i].next()) return true;
    tuples_[i].reset();
  }
  return false;
}
...
class MSConnectivityScore {
 ...
 private:
  ...
  bool check_assignment(NNGraph &G, unsigned int node_handle,
                        Assignment const &assignment, EdgeSet &picked) const;
...
MSConnectivityScore::MSConnectivityScore(
    const MSConnectivityRestraint::ExperimentalTree &tree, const PairScore *ps,
    double eps, MSConnectivityRestraint &restraint)
    : restraint_(restraint), ps_(ps), tree_(tree), eps_(eps) {
...
unsigned int MSConnectivityRestraint::add_composite(const Ints &composite) {
