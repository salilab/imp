...
ObjectsTemp PairConstraint::get_input_objects() const {
  return ObjectsTemp();
}

ObjectsTemp PairConstraint::get_output_objects() const {
  return ObjectsTemp();
}
...
void PairConstraint::show(std::ostream &out) const {
  out << "PairConstraint with ";
  if (f_) out << *f_;
  else out << "NULL";
  out << " and ";
  if (af_) out << *af_;
