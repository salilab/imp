/**
 *  \file MinimumPairScoreRestraint.cpp
 *  \brief The minimum score over a container.
 *
 *  This file is generated by a script (core/tools/make-container).
 *  Do not edit directly.
 *
 *  Copyright 2007-9 Sali Lab. All rights reserved.
 *
 */

#include "IMP/core/MinimumPairScoreRestraint.h"
#include "IMP/core/internal/MinimalSet.h"
#include <IMP/internal/container_helpers.h>
#include <IMP/internal/utility.h>


IMPCORE_BEGIN_NAMESPACE

MinimumPairScoreRestraint
::MinimumPairScoreRestraint(PairScore *f,
                                 PairContainer *c,
                                 unsigned int n,
                                 std::string name):
  Restraint(name),
  f_(f), c_(c), n_(n){
}

namespace {
  typedef internal::MinimalSet<double,
    PairContainer::ParticlePairIterator> MS;
  template <class It, class F>
  MS find_minimal_set(It b, It e, F *f, unsigned int n) {
    IMP_LOG(TERSE, "Finding lowest " << n << " of "
            << std::distance(b,e) << std::endl);
    MS bestn(n);
    for (It it= b; it != e; ++it) {
      double score= IMP::internal::ContainerTraits<ParticlePair>
                           ::evaluate(f, *it, NULL);

      if (bestn.can_insert(score)) {
        bestn.insert(score, it);
      }
    }
    return bestn;
  }
}

double MinimumPairScoreRestraint
::unprotected_evaluate(DerivativeAccumulator *da) const {
  MS bestn= find_minimal_set(c_->particle_pairs_begin(),
                             c_->particle_pairs_end(), f_.get(), n_);

  double score=0;
  for (unsigned int i=0; i< bestn.size(); ++i) {
    if (da) {
      IMP::internal::ContainerTraits<ParticlePair>::evaluate(f_.get(),
                                                     *bestn[i].second, da);
    }
    score+= bestn[i].first;
  }

  return score;
}

void MinimumPairScoreRestraint::show(std::ostream &out) const {
  out << "MinimumPairScoreRestraint over ";
  c_->show(out);
  out << " using function ";
  f_->show(out);
  out << std::endl;
}


ParticlesList MinimumPairScoreRestraint::get_interacting_particles() const
{
  MS bestn= find_minimal_set(c_->particle_pairs_begin(),
                             c_->particle_pairs_end(), f_.get(), n_);
  ParticlesList ret;
  for (unsigned int i=0; i< bestn.size(); ++i) {
    ParticlesList pt=IMP::internal::get_interacting_particles(*bestn[i].second,
                                                              f_.get());
    if (!pt.empty()) {
      ret.push_back(IMP::internal::get_union(pt));
    }
  }

  return ret;
}


ParticlesTemp MinimumPairScoreRestraint::get_read_particles() const
{
  return IMP::internal::get_read_particles(c_, f_.get());
}


ParticlesTemp MinimumPairScoreRestraint::get_write_particles() const
{
  return IMP::internal::get_write_particles(c_, f_.get());
}

IMPCORE_END_NAMESPACE
