...
double ExcludedVolumeRestraint::unprotected_evaluate(
  ...
  for (unsigned int i = 0; i < cur_list_.size(); ++i) {
    ret += ssps_->evaluate_index(
        get_model(), ParticleIndexPair(cur_list_[i][0], cur_list_[i][1]), da);
  }
  ...
  for (unsigned int i = 0; i < cur_list_.size(); ++i) {
    ...
    if (max < 0) {
      IMP_INTERNAL_CHECK(
          ...
          "I think it is bad, but it isn't: " << cur << " vs " << check);
      return cur;
    }
    ;
  }
  if (was_bad_ || get_if_moved() > 0) {
    ...
    IMP_IF_CHECK(base::USAGE_AND_INTERNAL) {
      if (ret > max) {
        ...
        IMP_INTERNAL_CHECK(
            (check - ret) < .1 * (check + ret) + .1,
            "Bad value computed 2: " << check << " vs " << ret << std::endl);
      }
    }
    return ret;
  } else {
    IMP_INTERNAL_CHECK(
        ...
        "Bad value computed 3: " << check << " vs " << cur << std::endl);
    return cur;
  }
}
...
Restraints ExcludedVolumeRestraint::do_create_decomposition() const {
  ...
  for (unsigned int i = 0; i < xyzrs_.size(); ++i) {
    for (unsigned int j = 0; j < i; ++j) {
      ret.push_back(IMP::create_restraint(
          ssps_.get(),
          ParticlePair(IMP::internal::get_particle(get_model(), xyzrs_[i]),
  ...
  for (unsigned int i = 0; i < rbs_.size(); ++i) {
    for (unsigned int j = 0; j < i; ++j) {
      ret.push_back(IMP::create_restraint(
          cpps.get(),
          ParticlePair(IMP::internal::get_particle(get_model(), rbs_[i]),
                       IMP::internal::get_particle(get_model(), rbs_[j]))));
      ret.back()->set_maximum_score(get_maximum_score());
      std::ostringstream oss;
      oss << get_name() << " " << i << " " << j;
