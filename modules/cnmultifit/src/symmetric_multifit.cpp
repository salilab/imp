...
void write_in_chimera_format(const multifit::FittingSolutionRecords &recs,
  ...
  for (unsigned int i = 0; i < recs.size(); i++) {
    ...
    algebra::Vector3D trans =
        recs[i].get_dock_transformation().get_translation();
    for (int j = 0; j < 3; j++) {
...
multifit::FittingSolutionRecords fit_models_to_density(
    em::DensityMap *dmap, const atom::Hierarchies &mhs,
    const multifit::FittingSolutionRecords &recs,
    const internal::Parameters &params, int num_sols_to_fit,
  ...
  boost::timer::progress_display show_progress(num_sols_to_fit + 1);
  for (int i = 0; i < recs_size && j < num_sols_to_fit; ++i) {
    ...
    if (aligner.score_alignment(mhs, max_allowed_diff) < 3) {
      cnmultifit::transform_cn_assembly(
          mhs, recs[i].get_dock_transformation().get_inverse());
      continue;
    }
    ++j;
    ++show_progress;
    ...
    em::FittingSolutions asmb_fits =
        fit_cn_assembly(mhs, dn_symm_deg, dmap, 1., aligner, sample_translation,
...
void do_all_fitting(const std::string param_filename,
  ...
  std::cout << "symmetry degree (cn,dn): " << cn_symm_deg << "," << dn_symm_deg
            << std::endl;
  ...
  multifit::FittingSolutionRecords fitted_recs =
      fit_models_to_density(dmap, mhs, recs, params, num_sols_to_fit, false);
  std::sort(fitted_recs.begin(), fitted_recs.end(), sort_by_cc());
  std::fstream out;
  int i = 0;
