...
namespace {
// return x%y
int my_mod(int x, int y) { return (x % y + y) % y; }
...
algebra::Transformation3Ds generate_d2_translations_along_symm_axis(
  ...
  algebra::Transformation3D translation(algebra::get_identity_rotation_3d(),
                                        symm_axis * symm_axis_val / 2);
  all_trans.push_back(translation);
  return all_trans;
}

em::FittingSolutions fast_cc(em::DensityMap *dmap1, em::DensityMap *dmap2,
...
em::FittingSolutions fast_cc_translation(
    em::DensityMap *dmap1, em::DensityMap *dmap2,
    algebra::Transformation3Ds trans_on_dmap2) {
  em::FittingSolutions fits;
  float threshold = dmap2->get_header()->dmin + em::EPS;
  float score;
...
em::FittingSolutions symmetry_local_fitting(atom::Hierarchies mhs,
  ...
  if (dn_symm_deg == 1) {
    ...
    std::cerr << "Dn of degree " << dn_symm_deg << " not supported"
              << std::endl;
    exit(0);
  }
  em::FittingSolutions trans_fit_sols =
      em::compute_fitting_scores(ps, dmap, refined_trans, true);
  trans_fit_sols.sort();
  ...
  for (int i = 0; i < std::min(num_of_trans_to_consider,
    ...
    em::FittingSolutions rot_fit_sols =
        em::compute_fitting_scores(ps, dmap, refined_rot_trans, true);
    rot_fit_sols.sort();
...
em::DensityMap *build_cn_dens_assembly(
  ...
  PointerMember<em::DensityMap> ret(em::create_density_map(
      asmb_dens_header.get_nx(), asmb_dens_header.get_ny(),
      asmb_dens_header.get_nz(), asmb_dens_header.get_spacing()));
  ...
  for (int i = 0; i < symm_deg; i++) {
    PointerMember<em::DensityMap> trans_subunit =
...
em::FittingSolutions fit_cn_assembly(atom::Hierarchies mhs, int /*dn_symm_deg*/,
  ...
  em::FittingSolutions coarse_sols =
      em::compute_fitting_scores(ps, dmap, alignments, true);  // false
  // create a rigid body
  ...
  for (int i = 0; i < coarse_sols.get_number_of_solutions(); i++) {
    if (is_nan(coarse_sols.get_score(i))) continue;
...
em::FittingSolutions fit_cn_assembly(em::DensityMap *asmb_map,
  ...
  for (int i = 0; i < std::min(3,  // TODO - should be a parameter
                               translations_fit_sols.get_number_of_solutions());
       i++) {
...
float cn_symm_score(atom::Hierarchies mhs, const algebra::Vector3D &center,
  ...
  algebra::Rotation3D rot =
      algebra::get_rotation_about_axis(direction, 2 * PI / symm_deg);
...
multifit::FittingSolutionRecords prune_by_pca(
  ...
  PointerMember<em::DensityMap> dmap =
      em::read_map(par.get_density_map_filename(), new em::MRCReaderWriter());
...
float get_cn_rmsd(atom::Hierarchies mh1, atom::Hierarchies mh2) {
  ...
  for (unsigned int j = 0; j < mh2.size(); j++) {
    ...
    float rmsd = atom::get_rmsd(core::XYZs(core::get_leaves(mh2[j])), mh11_xyz);
    if (rmsd < min_dist_11) {
      min_dist_11 = rmsd;
      closest_to_11 = j;
    }
    rmsd = atom::get_rmsd(core::XYZs(core::get_leaves(mh2[j])), mh12_xyz);
    if (rmsd < min_dist_12) {
      min_dist_12 = rmsd;
      closest_to_12 = j;
    }
  ...
  for (int j = 0; j < m; j++) {
    int j2 =
        my_mod(sign * j + closest_to_11, m);  // we also have negative numbers
