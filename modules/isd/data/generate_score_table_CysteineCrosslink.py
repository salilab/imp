...
def get_cross_link_data(directory,filename,(distmin,distmax,ndist),
    ...
    try:
        ...
        xlmsdata=IMP.isd2.CrossLinkData(dist_grid,omega_grid,sigma_grid,xpot,pot,10.0,20.0)
    except:
...
for fexp in [0,0.25,0.5,0.75,1.0]:
    ...
    #erf1dataunbiased=IMP.isd2.CrossLinkMSData(dist_grid,omega_grid,sigma_grid,10.0)
    datacyst=IMP.isd2.CysteineCrossLinkData(fexp,fmod_grid,omega2_grid,beta_grid)

    cystrest=IMP.isd2.CysteineCrossLinkRestraint(beta,sigma,epsilon,weight,crossdata,datacyst)
            ...
            for ibeta in betalist:
                for iweight in weightlist:
                    for iepsilon in epsilonlist:
                        ...
                        cysterest.get_probability(),cysterest.get_standard_error(),cysterest.get_frequencies())+"\n"
                        output+=s
