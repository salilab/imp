...
initpdb = "data/extended.pdb"
charmmtop = "data/top.lib"
charmmpar =  "data/par.lib"
noes = "data/1edg_NOE_full_4A_60pc.tbl"
saxs = 'data/iq.dat'
#export the files in a local tmp directory
filelist=[initpdb,charmmtop,charmmpar,noes,
            saxs, 'shared_functions.py'] #add whatever you want
#prefix of output files 
nums=[[os.path.join(outfolder,'r%02d' % (i+1))] for i in xrange(nreps)]
#thermalization (mc parameters stay fixed)
n_therm = 10 #number of loops, where temperatures are scaled to target value
n_hmc_therm = 100 #number of steps per loop
...
def main():
    ...
    mkdir_p(tmpdir)
    mkdir_p(outfolder)
    requests = grid.broadcast(sfo_id, 'init_model', tmpdir, initpdb,noes,saxs)
    ...
    for i in range(n_therm):
        ...
        sys.stdout.flush()
        grid.gather(grid.scatter(sfo_id, 'set_temp', 
            [ n_therm/float(i+1) * l for l in lambdas ]))
        grid.gather(grid.broadcast(sfo_id, 'do_hmc', n_hmc_therm))
    ...
    for i in range(n_gibbs1):
        ...
        sys.stdout.flush()
        #print " md"
        grid.gather(grid.broadcast(sfo_id, 'do_hmc', n_hmc))
        #print " mc"
    ...
    for i in range(n_gibbs2):
        ...
        grid.gather(grid.broadcast(sfo_id, 'do_hmc', n_hmc))
        #print " mc"
        grid.gather(grid.broadcast(sfo_id, 'do_hmc', n_mc))
        #print " swaps"
