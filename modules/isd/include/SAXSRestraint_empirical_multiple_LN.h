...
#ifndef IMPISD_SAXS_RESTRAINT_EMPIRICAL_MULTIPLE_LN_H
#define IMPISD_SAXS_RESTRAINT_EMPIRICAL_MULTIPLE_LN_H
...
//  weight. Empirical Bayes, multiple sigma.
/** \ingroup exp_restraint

    The likelihood of this model is a lognormal. Assume M data points.
    \f[p(D | X,{\sigma},\gamma, I) = \frac{1}{(2\pi)^{M/2}} 
    \prod_{q=q_{min}}^{q_{max}} \frac{1}{\sigma_{exp}(q) \sigma(q) I_{exp}(q)}
    \exp left[ - \frac{1}{2\sigma^2(q) \sigma^2_{exp}(q)} 
 ...
 */
class IMPISDEXPORT SAXSRestraint_empirical_multiple_LN : public ISDRestraint
{
 public:
  //! Constructor
  /**
  ...
  */
  SAXSRestraint_empirical_multiple_LN(const Particles& particles, const Scales& sigmas, const
          Scale& gamma, const saxs::Profile& exp_profile, saxs::FormFactorType ff_type = saxs::HEAVY_ATOMS);

  IMP_RESTRAINT(SAXSRestraint_empirical_multiple_LN);
...
#endif  /* IMPISD_SAXS_RESTRAINT_EMPIRICAL_MULTIPLE_LN_H */
