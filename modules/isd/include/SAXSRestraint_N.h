...
#ifndef IMPISD_SAXS_RESTRAINT_N_H
#define IMPISD_SAXS_RESTRAINT_N_H
...
/** \ingroup exp_restraint
    ...
    \f[p(D | X,\alpha,\gamma, I) = \frac{1}{(\sqrt{2\pi}\alpha)^M} 
    \prod_{q=q_{min}}^{q_{max}} \frac{1}{\sigma{exp}(q)}
    \exp left[ - \frac{1}{2\alpha^2\sigma^2_{exp}(q)} 
 ...
 */
class IMPISDEXPORT SAXSRestraint_N : public ISDRestraint
{
 public:
  //! Constructor
  /**
     \param[in] particles The particles that generated the profile, usually atoms of a protein.
     \param[in] alpha An ISD Scale particle that will be \f$\alpha\f$
  ...
  */
  SAXSRestraint_N(const Particles& particles, const Scale& alpha,
          const Scale& gamma, const saxs::Profile& exp_profile,
          saxs::FormFactorType ff_type = saxs::HEAVY_ATOMS);

  IMP_RESTRAINT(SAXSRestraint_N);
...
#endif  /* IMPISD_SAXS_RESTRAINT_N_H */
