...
Particles get_particles(Model* m) {
    ...
    atom::Hierarchy mp =
        atom::read_pdb(saxs::get_example_path("6lyz.pdb"), m, sel);
...
ArrayCoord get_coords(Model* m, Particles p) {
    ...
    for (unsigned i = 0; i < p.size(); ++i) {
        ...
        for (unsigned j = 0; j < 3; ++j) coords(i, j) = tmp[j];
    }
    return coords;
}

IMP_Eigen::ArrayXd get_form_factors(Particles p) {
    saxs::FormFactorTable* fft = saxs::default_form_factor_table();
    IMP_Eigen::ArrayXd ffs(p.size());
    for (unsigned i = 0; i < p.size(); ++i)
...
IMP_Eigen::ArrayXd get_distances(IMP_Eigen::Array3d atom, ArrayCoord coords) {
    return (coords.rowwise() - atom.transpose())
        .square()
        .rowwise()
        .sum()
        .sqrt();
...
IMP_Eigen::ArrayXd compute_profile(ArrayCoord coords, IMP_Eigen::ArrayXd ffs,
                                   SincFunction& sf, double qmin = 0.,
                                   double qmax = 0.5, unsigned num = 101) {
    // I(q) = E^2(q) sum_{i<=j} fi(0) fj(0) sinc(q dij)
    // E^2(q) = exp(- q^2 * 0.23 )
    IMP_Eigen::ArrayXd profile(num);
    // add autocorrelation, dij = 0
    profile.setConstant(ffs.square().sum());
    //
    // add other terms
    for (unsigned i = 0; i < ffs.rows() - 1; ++i) {
        IMP_Eigen::ArrayXd dists =
            get_distances(coords.row(i), coords.bottomRows(ffs.rows() - i - 1));
...
IMP_Eigen::ArrayXd compute_blurred_profile(ArrayCoord coords,
                                           IMP_Eigen::ArrayXd ffs, double tau,
                                           SincFunction& sf, double qmin = 0.,
                                           double qmax = 0.5,
                                           unsigned num = 101) {
    IMP_Eigen::ArrayXd profile =
        compute_profile(coords, ffs, sf, qmin, qmax, num);
    for (unsigned nq = 0; nq < num; ++nq) {
        double q = qmin + (qmax - qmin) * nq / (num - 1);
...
IMP_Eigen::ArrayXXd compute_autocovariance(ArrayCoord coords,
                                           IMP_Eigen::ArrayXd ffs, double tau,
                                           SincCosFunction& sf,
                                           double qmin = 0., double qmax = 0.5,
                                           unsigned num = 101) {
    ...
    //
    IMP_Eigen::ArrayXXd autocov = IMP_Eigen::ArrayXXd::Zero(num, num);
    for (unsigned k = 0; k < ffs.size() - 1; ++k) {
        IMP_Eigen::ArrayXd dists =
            get_distances(coords.row(k), coords.bottomRows(ffs.rows() - k - 1));
    ...
    //
    IMP_Eigen::ArrayXd factor =
        (-IMP_Eigen::ArrayXd::LinSpaced(num, qmin, qmax).square() * 0.23).exp();
    autocov.colwise() *= factor;
    autocov.rowwise() *= factor.transpose();
    // factor 2 for the half-sum
    return IMP_Eigen::MatrixXd((2 * (2 * tau * tau) * autocov)
                                   .matrix()
                                   .selfadjointView<IMP_Eigen::Upper>())
        .array();
}

double get_rho(IMP_Eigen::Array3d ck, IMP_Eigen::Array3d cl,
               IMP_Eigen::Array3d cn, double tau) {
    // rho(dkl,dkn) = (2*dkl.dkn + 3*tau^2)/(4*dkl*dkn)
    double scal = ((cl - ck) * (cn - ck)).sum();
    double den =
        4 * std::sqrt((cl - ck).square().sum() * (cn - ck).square().sum());
    return (2 * scal + 3 * tau * tau) / den;
}

IMP_Eigen::ArrayXXd compute_crosscovariance(ArrayCoord coords,
                                            IMP_Eigen::ArrayXd ffs, double tau,
                                            SincCosFunction& sf,
                                            double qmin = 0., double qmax = 0.5,
                                            unsigned num = 101) {
    ...
    // sigma(x) = (sinc(x)-cos(x))/x
    // rho(dkl,dkn) = (2*dkl.dkn + 3*tau^2)/(4*dkl*dkn)
    IMP_Eigen::ArrayXXd ccov = IMP_Eigen::ArrayXXd::Zero(num, num);
    for (unsigned k = 0; k < ffs.size() - 1; ++k) {
        IMP_Eigen::ArrayXd dists = get_distances(coords.row(k), coords);
        for (unsigned l = 0; l < dists.rows() - 1; ++l) {
            ...
            for (unsigned n = l + 1; n < dists.rows(); ++n) {
                ...
                double rho =
                    get_rho(coords.row(k), coords.row(l), coords.row(n), tau);
                for (unsigned nqi = 0; nqi < num; ++nqi) {
                    double qi = qmin + (qmax - qmin) * nqi / (num - 1);
                    ...
                    for (unsigned nqj = nqi; nqj < num; ++nqj) {
                        ...
                        ccov(nqi, nqj) +=
                            fkln * rho * qi * qj * (sil * sjn + sjl * sin);
                    }  // qj
                }      // qi
            }          // n
        }              // l
    ...
    return IMP_Eigen::MatrixXd(
        (0.5 * (tau * tau) * ccov).matrix().selfadjointView<IMP_Eigen::Upper>())
        .array();
}

IMP_Eigen::ArrayXXd compute_crosscovariance_approx(
    ArrayCoord coords, IMP_Eigen::ArrayXd ffs, double tau, SincCosFunction& sf,
...
IMP_Eigen::ArrayXXd compute_covariance_numerical(
    ArrayCoord coords, IMP_Eigen::ArrayXd ffs, double tau, SincFunction& sinc,
    double qmin = 0., double qmax = 0.5, unsigned num = 101,
    unsigned numloops = 100) {
    // compute perturbed profiles
    IMP_Eigen::ArrayXXd profiles(num, numloops);
    for (unsigned n = 0; n < numloops; ++n) {
        ArrayCoord newcoords = perturb_particles(coords, tau);
        profiles.col(n) =
            compute_profile(newcoords, ffs, sinc, qmin, qmax, num);
    }
    // compute covariance matrix
    IMP_Eigen::ArrayXd meanprof(
        compute_profile(coords, ffs, sinc, qmin, qmax, num));
    IMP_Eigen::MatrixXd residuals(profiles.colwise() - meanprof);
    IMP_Eigen::ArrayXXd covmat((residuals * residuals.transpose()) /
                               double(numloops));
    return covmat;
}

IMP_Eigen::ArrayXXd make_relative(IMP_Eigen::ArrayXXd mat,
                                  IMP_Eigen::ArrayXd vec) {
