...
int main(int argc, char **argv) {
    ...
    start = clock();
    IMP_Eigen::ArrayXXd covmat = compute_covariance_numerical(
        coords, ffs, tau, sinc, qmin, qmax, num, numloops);
    end = clock();
    time = difftime(end, start) / CLOCKS_PER_SEC;
    //
    // output profile
    std::cout << "profile " << time << " " << (time / double(numloops))
              << std::endl;
    myfile.open("profiles.dat");
    IMP_Eigen::ArrayXXd residuals(
        covmat - (profile.matrix() * profile.matrix().transpose()).array());
