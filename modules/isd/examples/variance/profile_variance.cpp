...
int main(int argc, char **argv) {
    IMP::base::setup_from_argv(argc, argv, "Compute a SAXS profile");
    ...
    IMP_Eigen::ArrayXd profile =
        compute_blurred_profile(coords, ffs, tau, sinc, qmin, qmax, num);
    end = clock();
    ...
    start = clock();
    IMP_Eigen::ArrayXXd autocov =
        compute_autocovariance(coords, ffs, tau, sigma, qmin, qmax, num);
    end = clock();
    time = difftime(end, start) / CLOCKS_PER_SEC;
    IMP_Eigen::ArrayXXd autorel = make_relative(autocov, profile);
    ...
    start = clock();
    IMP_Eigen::ArrayXXd crosscov =
        compute_crosscovariance(coords, ffs, tau, sigma, qmin, qmax, num);
    end = clock();
    time = difftime(end, start) / CLOCKS_PER_SEC;
    IMP_Eigen::ArrayXXd crossrel = make_relative(crosscov, profile);
    ...
    IMP_Eigen::ArrayXXd acrosscov = compute_crosscovariance_approx(
        coords, ffs, tau, sigma, qmin, qmax, num);
    IMP_Eigen::ArrayXXd acrossrel = make_relative(acrosscov, profile);
    ...
    IMP_Eigen::ArrayXXd delta = ((crosscov - acrosscov) / crosscov).abs();
    IMP_Eigen::ArrayXXd deltarel = ((crossrel - acrossrel) / crossrel).abs();
    std::cout << "error abs " << delta.maxCoeff() << " " << delta.mean()
              << std::endl;
    std::cout << "error rel " << deltarel.maxCoeff() << " " << deltarel.mean()
              << std::endl;

    // full matrix
    IMP_Eigen::ArrayXXd covmat = autocov + crosscov;
    IMP_Eigen::ArrayXXd covrel = make_relative(covmat, profile);
    IMP_Eigen::ArrayXXd deltacross = (crosscov / covmat).abs();
    IMP_Eigen::ArrayXXd deltacrossrel = ((covrel - autorel) / covrel).abs();
    ...
    IMP_Eigen::ArrayXXd contribs(num, 5);
    contribs.col(0) = IMP_Eigen::ArrayXd::LinSpaced(num, qmin, qmax);
