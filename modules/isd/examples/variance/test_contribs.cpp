...
int main(int argc, char **argv) {
    ...
    IMP_Eigen::ArrayXXd contribs(num, 8);
    IMP_Eigen::ArrayXd tauvals(
        IMP_Eigen::ArrayXd::LinSpaced(num, taumin, taumax));
    for (unsigned i = 0; i < num; ++i) {
        ...
        contribs(i, 2) = compute_autocovariance(
            coords, ffs, tau, sigma, qtarget, (qtarget + 0.1), 2)(0, 0);
        // compute crosscovariance
        contribs(i, 3) = compute_crosscovariance(
            coords, ffs, tau, sigma, qtarget, (qtarget + 0.1), 2)(0, 0);
        // full matrix
        contribs(i, 1) = contribs(i, 2) + contribs(i, 3);

        // compute perturbed profiles: 300 three times
        contribs(i, 4) = compute_covariance_numerical(
            coords, ffs, tau, sinc, qtarget, (qtarget + 0.1), 2, 300)(0, 0);
        contribs(i, 5) = compute_covariance_numerical(
            coords, ffs, tau, sinc, qtarget, (qtarget + 0.1), 2, 300)(0, 0);
        contribs(i, 6) = compute_covariance_numerical(
            coords, ffs, tau, sinc, qtarget, (qtarget + 0.1), 2, 300)(0, 0);

        // compute perturbed profiles: 1000
        contribs(i, 7) = compute_covariance_numerical(
            coords, ffs, tau, sinc, qtarget, (qtarget + 0.1), 2, 1000)(0, 0);
    }
