...
class TestFretRestraint(IMP.test.TestCase):
    ...
    def getFM0(self):
        ...
        p_ad = Pbl * (1.0 - Pbl)
        p_da = (1.0 - Pbl) * Pbl
        sumFi_ave = p_aa * sumFi_aa + p_dd * \
            sumFi_dd + p_ad * sumFi_ad + p_da * sumFi_da
        fretr = (
            Ida * sumFi_aa + 2.0 + kda * (
                1.0 - sumFi_aa)) / (
            Ida * sumFi_ave + 2.0)
        return fretr
    ...
    def getFM1(self):
        ...
        sumFi_ave = p_a * sumFi_a0 + p_d * \
            sumFi_d0 + p_a * sumFi_a1 + p_d * sumFi_d1
        fretr = (
            Ida * sumFi + 1.0 + kda * (
                2.0 - sumFi)) / (
            Ida * sumFi_ave + 1.0)
    ...
    def getP(self, fmod):
        ...
        prob = math.sqrt(
            2.0) * sigma / self.fexp / math.pi / (
            log_eps * log_eps + 2.0 * sigma * sigma)
        return prob
