...
bool test_1D(int N){
    ...
    //Precision matrix
    Array2D<double> P(1,1,rand()*10);

    //IMP_NEW(IMP::isd::FNormal, fn, (FA[0][0],JA,FM[0],sqrt(1/P[0][0])));
    ...
    double expected=pow(P[0][0]/(2*IMP::PI),N/2.0) * exp(
            -0.5*P[0][0]*( N*IMP::square(sample_mean - FM[0]) + sum_squares ));
    ...
    double expected = -double(N)*P[0][0]*(sample_mean - FM[0]);
    //PRINT("FM " << observed[0] << " " << expected << " " << fn->evaluate_derivative_FM());
    if (naeq(observed[0],expected)) FAIL("evaluate_derivative_FM");
    }

    {
    //test evaluate_derivative_P()
    Array2D<double> observed = mv->evaluate_derivative_P();
    double expected = -N/(2.0*P[0][0]) + 0.5 * ((double)N*IMP::square(sample_mean - FM[0]) + sum_squares);
    //PRINT("density " << observed[0][0] << " " << expected << " " << fn->evaluate_derivative_sigma()/(-2*pow(P[0][0],1.5)));
    if (naeq(observed[0][0],expected)) FAIL("evaluate_derivative_P");
    }
...
bool test_2D(){
    ...
    {
    //test_evaluate_derivative_P
    Array2D<double> observed=mv->evaluate_derivative_P();
    double expected=0.5*(IMP::square(FA[0][0]-FM[0])-sigma1*sigma1);
    if (naeq(observed[0][0],expected)) FAIL("derivative_P[0][0]");
    expected=0.5*(IMP::square(FA[0][1]-FM[1])-sigma2*sigma2);
    if (naeq(observed[1][1],expected)) FAIL("derivative_P[1][1]");
    expected=0.5*((FA[0][0]-FM[0])*(FA[0][1]-FM[1])-rho*sigma1*sigma2);
...
bool test_degenerate(){
    ...
    IMP_NEW(IMP::isd::MultivariateFNormalSufficient, mv, (FA,JA,FM,P));
    mv->set_was_used(true);
    IMP_NEW(IMP::isd::FNormal, fn, (FA[0][0],JA,FM[0],sqrt(1.0/P[0][0])));
    fn->set_was_used(true);
    ...
    expected=expected/(-2.0*pow(P[0][0],1.5));
...
//test with N MultivariateFNormalSufficient instances
...
//test with M=1 and N=2, varying mu
bool test_mu(int N){
    //observation matrix
