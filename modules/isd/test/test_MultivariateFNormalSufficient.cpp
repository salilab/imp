...
bool test_sanity() {
...

    {
        ...
        double expected =
            mv->get_minus_exponent() + mv->get_minus_log_normalization();
        if (naeq(observed, expected))
            FAIL("evaluate = exponent + normalization");
    }
...
bool test_degenerate_N2M1() {
...

    {
        ...
        double observed = mv->evaluate_derivative_FM()(0);
        double expected =
            fn->evaluate_derivative_FM() + fn2->evaluate_derivative_FM();
        if (naeq(observed, expected)) FAIL("evaluate_derivative_FM");
    }
...
bool test_1D(int N) {
...

    {
        ...
        double expected =
            double(N) * log((sqrt(2 * IMP::PI) * sigma)) +
            1 / (2 * sigma * sigma) *
...
bool test_2D() {
...

    {
        ...
        double observed = mv->evaluate();
        double expected =
            log(2 * IMP::PI * sigma1 * sigma2 * sqrt(1 - rho * rho)) +
            1 / (2 * (1 - rho * rho)) *
                (IMP::square(FA(0, 0) - FM(0)) / (sigma1 * sigma1) +
                 IMP::square(FA(0, 1) - FM(1)) / (sigma2 * sigma2) -
                 2 * rho * (FA(0, 0) - FM(0)) * (FA(0, 1) - FM(1)) /
                     (sigma1 * sigma2));
        if (naeq(observed, expected)) FAIL("evaluate");
    }

    {
        ...
        double observed = mv->density();
        double expected =
            1 / (2 * IMP::PI * sigma1 * sigma2 * sqrt(1 - rho * rho)) *
            exp(-1 / (2 * (1 - rho * rho)) *
                (IMP::square(FA(0, 0) - FM(0)) / (sigma1 * sigma1) +
                 IMP::square(FA(0, 1) - FM(1)) / (sigma2 * sigma2) -
                 2 * rho * (FA(0, 0) - FM(0)) * (FA(0, 1) - FM(1)) /
...

    {
        ...
        expected =
            ((FA(0, 1) - FM(1)) * sigma1 + (FM(0) - FA(0, 0)) * rho * sigma2) /
...

    {
        ...
        expected = 0.5 / IMP::square(det) *
                   (Sigma(0, 1) * (-det + Sigma(1, 1) * IMP::square(eps1) +
                    ...
                    eps1 * eps2 *
                        (IMP::square(Sigma(1, 0)) + Sigma(0, 0) * Sigma(1, 1)));
...
bool test_factor() {
    ...
    IMP_NEW(IMP::isd::MultivariateFNormalSufficient, mv,
            (FA, JA, FM, factor * factor * Sigma));
    IMP_NEW(IMP::isd::MultivariateFNormalSufficient, mvf,
            (FA, JA, FM, Sigma, factor));
    // mv->set_was_used(true);
...

    {
        ...
        for (unsigned i = 0; i < 100; i++)
            if (naeq(observed(i), expected(i)))
                FAIL("derivative_FM coef " << i);
    }

    // change factor and test again
    factor = 10 * (rand() + 1);

    IMP_NEW(IMP::isd::MultivariateFNormalSufficient, mv2,
            (FA, JA, FM, factor * factor * Sigma));
