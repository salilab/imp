...
bool test_degenerate(){
    ...
    IMP_NEW(IMP::isd::MultivariateFNormalSufficientSparse, mv, 
            (FA,JA,FM,Sigma,&c_));
    //mv->set_was_used(true);
    IMP_NEW(IMP::isd::FNormal, fn, (FA(0,0),JA,FM(0),sqrt(MatrixXd(Sigma)(0,0))));
...
bool test_degenerate_N2M1(){
    ...
    //mv->set_was_used(true);
    IMP_NEW(IMP::isd::FNormal, fn, (FA(0,0),JA,FM(0),sqrt(MatrixXd(Sigma)(0,0))));
    IMP_NEW(IMP::isd::FNormal, fn2, (FA(1,0),JA,FM(0),sqrt(MatrixXd(Sigma)(0,0))));
    //fn->set_was_used(true);
    ...
    //double ss = 0.5*(IMP::square(FA(0,0) - mean) + IMP::square(FA(1,0) - mean))/MatrixXd(Sigma)(0,0);
...
bool test_degenerate_N1M2(){
    ...
    //mv->set_was_used(true);
    IMP_NEW(IMP::isd::FNormal, fn, (FA(0,0),JA,FM(0),sqrt(MatrixXd(Sigma)(0,0))));
    IMP_NEW(IMP::isd::FNormal, fn2, (FA(0,1),JA,FM(1),sqrt(MatrixXd(Sigma)(1,1))));
    //fn->set_was_used(true);
...
bool test_1D(int N){
    ...
    double expected=pow(2*IMP::PI*MatrixXd(Sigma)(0,0),-N/2.0) * exp(
            -0.5/MatrixXd(Sigma)(0,0)*( (double)N*IMP::square(sample_mean - FM(0)) + sum_squares ));
...
bool test_2D(){
    ...
    double expected=0.5/IMP::square(det) 
        * (   IMP::square(MatrixXd(Sigma)(1,1)) * (MatrixXd(Sigma)(0,0) - IMP::square(eps1)) 
            - IMP::square(MatrixXd(Sigma)(1,0)) * (MatrixXd(Sigma)(1,1) + IMP::square(eps2))
    ...
    expected=0.5/IMP::square(det)
        * (   IMP::square(MatrixXd(Sigma)(0,0)) * (MatrixXd(Sigma)(1,1) - IMP::square(eps2)) 
            - IMP::square(MatrixXd(Sigma)(1,0)) * (MatrixXd(Sigma)(0,0) + IMP::square(eps1))
            + 2*MatrixXd(Sigma)(1,0)*MatrixXd(Sigma)(0,0)*eps2*eps1 );
    if (naeq(observed[3],expected)) FAIL("derivative_Sigma(1,1)");
    expected=0.5/IMP::square(det)
        * ( MatrixXd(Sigma)(1,0)*(-det+MatrixXd(Sigma)(1,1)*IMP::square(eps1)+MatrixXd(Sigma)(0,0)*IMP::square(eps2))
            - eps1*eps2*(IMP::square(MatrixXd(Sigma)(1,0))+MatrixXd(Sigma)(0,0)*MatrixXd(Sigma)(1,1)) );
