...
import IMP.saxs
import IMP.atom
from IMP.isd import Scale,SAXSRestraint_empirical_multiple_LN

#unit testing framework
import IMP.test

class SAXSRestraint_empirical_multiple_LN_Test(IMP.test.TestCase):
    """test the ISD SAXS empirical Lognormal multiple sigma Restraint"""
    ...
    def setUp(self):
        ...
        self.sr = IMP.isd.SAXSRestraint_empirical_multiple_LN(particles,
    ...
    def compute_prob(self):
        ...
        for i in xrange(self.exp_profile.size()):
            ...
            expected *= \
                1/(sqrt(2*pi)*iexp*sigmas[i]*err)*exp(-1/(2*(sigmas[i]*err)**2)*log(iexp/(gamma*icalc))**2)
        return expected
    ...
    def compute_ene(self):
        ...
        for i in xrange(self.exp_profile.size()):
            ...
            err=self.exp_profile.get_error(i)
            expected += \
                log(sqrt(2*pi)*iexp*sigmas[i]*err)+1/(2*(sigmas[i]*err)**2)*log(iexp/(gamma*icalc))**2
        return expected
