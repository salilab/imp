...
import IMP.saxs
import IMP.atom
from IMP.isd import Scale,SAXSRestraint_sliding_LN

#unit testing framework
import IMP.test

class SAXSRestraint_sliding_LN_Test(IMP.test.TestCase):
    ...
    def setUp(self):
        ...
        self.sr = IMP.isd.SAXSRestraint_sliding_LN(particles, self.gamma, self.alphag,
    ...
    def compute_prob(self):
        ...
        for i in xrange(self.exp_profile.size()):
            ...
            expected *= \
                1/(sqrt(2*pi)*iexp*err)*exp(-1/(2*err**2)*log(iexp/(gamma*icalc))**2)
        return expected
    ...
    def compute_ene(self):
        ...
        for i in xrange(self.exp_profile.size()):
            ...
            if q<=q0:
                ...
                err = err/sqrt(alphab*q)
            expected += \
                log(sqrt(2*pi)*iexp*err)+1/(2*err**2)*log(iexp/(gamma*icalc))**2
        return expected
