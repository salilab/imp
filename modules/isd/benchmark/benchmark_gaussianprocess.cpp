...
IMP::kernel::Model *do_setup(std::string profile, unsigned subs, unsigned cut) {
  ...
  IMP_NEW(GeneralizedGuinierPorodFunction, mean,
          (particles[0].get_particle(), particles[1].get_particle(),
           particles[2].get_particle(), particles[3].get_particle(),
           particles[4].get_particle()));
  IMP_NEW(Covariance1DFunction, covariance,
          (particles[5].get_particle(), particles[6].get_particle(), 2.0));
  IMP_NEW(GaussianProcessInterpolation, gpi,
          (qvals, data[1], data[2], 10, mean, covariance, particles[7]));
...
int main(int argc, char **argv) {
  IMP::base::setup_from_argv(argc, argv, "Benchmark a gaussian process");
  if (cut == -1) {
