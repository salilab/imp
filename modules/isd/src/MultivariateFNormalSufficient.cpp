...
MultivariateFNormalSufficient::MultivariateFNormalSufficient(
  ...
  N_ = FX.rows();
  M_ = FX.cols();
  IMP_LOG_TERSE("MVN: direct init with N=" << N_ << " and M=" << M_
                                           << std::endl);
  IMP_USAGE_CHECK(N_ > 0,
                  "please provide at least one observation per dimension");
  IMP_USAGE_CHECK(M_ > 0, "please provide at least one variable");
  set_factor(factor);
  set_FM(FM);
  set_FX(FX);
  set_jacobian(JF);
...
MultivariateFNormalSufficient::MultivariateFNormalSufficient(
  ...
  N_ = Nobs;
  M_ = Fbar.rows();
  IMP_LOG_TERSE("MVN: sufficient statistics init with N=" << N_ << " and M="
                                                          << M_ << std::endl);
  IMP_USAGE_CHECK(N_ > 0,
                  "please provide at least one observation per dimension");
  IMP_USAGE_CHECK(M_ > 0, "please provide at least one variable");
...
double MultivariateFNormalSufficient::density() const {
  ...
  if (N_ == 1) {
    ...
    d = get_norms()[0] * get_jacobian() *
        exp(-0.5 * (trace_WP() + N_ * get_mean_square_residuals()) /
            IMP::square(factor_));
  }
...
double MultivariateFNormalSufficient::evaluate() const {
  ...
  if (N_ == 1) {
    ...
    e += 0.5 * (trace_WP() + double(N_) * get_mean_square_residuals()) /
         IMP::square(factor_);
  }
...
void MultivariateFNormalSufficient::set_Fbar(const IMP_Eigen::VectorXd& Fbar) {
  if (Fbar.rows() != Fbar_.rows() || Fbar.cols() != Fbar_.cols() ||
      Fbar != Fbar_) {
    if (Fbar.rows() != M_) {
      IMP_THROW("size mismatch for Fbar: got " << Fbar.rows() << " instead of "
                                               << M_,
                ModelException);
    }
    Fbar_ = Fbar;
...
MultivariateFNormalSufficient::get_ldlt() const {
  if (!flag_ldlt_) {
    ...
    if (!ldlt.isPositive()) {
      ...
      IMP_THROW("Sigma matrix is not positive semidefinite!", ModelException);
    }
...
std::vector<double> MultivariateFNormalSufficient::get_norms() const {
  if (!flag_norms_) {
    ...
    double norm =
        pow(2 * IMP::PI * IMP::square(factor_), -double(N_ * M_) / 2.0) *
        exp(-double(N_) / 2.0 * logDetSigma);
    double lnorm =
        double(N_ * M_) / 2 * log(2 * IMP::PI * IMP::square(factor_)) +
        double(N_) / 2 * logDetSigma;
    IMP_LOG_TERSE("MVN:   norm = " << norm << " lnorm = " << lnorm
                                   << std::endl);
    const_cast<MultivariateFNormalSufficient*>(this)->set_norms(norm, lnorm);
  }
...
double MultivariateFNormalSufficient::get_log_generalized_variance() const {
  return - double(N_ * M_) * log(2 * IMP::PI) + 2*get_norms()[1];
}
