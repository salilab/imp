 ...
 add_contribution(Particles p1, Particles p2)
{
  if(get_number_of_contributions()==0) use_CA_=false;
    ...
    ModelException);}
  if(p1.size()!=3 || p2.size()!=3) {IMP_THROW("Exactly 3 CA are needed!",
    ModelException);}
...
double CysteineCrossLinkRestraint::get_standard_error() const
{
 ...
 if (constr_type_==1){beta=ccldata_->get_omega(freq,beta);}
...
// first versor
  algebra::Vector3D a=(core::XYZ(ps[0]).get_coordinates()-x1).get_unit_vector();

// auxiliary vector
  algebra::Vector3D b=(core::XYZ(ps[2]).get_coordinates()-x1).get_unit_vector();
...
double CysteineCrossLinkRestraint::get_normal_pdf
 (double mean, double sigma, double x) const
{
...
double CysteineCrossLinkRestraint::
