...
NOERestraint::NOERestraint(Model *m, Particle *p0, Particle *p1,
                           Particle *sigma, Particle *gamma, double Vexp)
    : Restraint(m, "NOERestraint%1%"),
      p0_(p0),
      p1_(p1),
      sigma_(sigma),
      gamma_(gamma),
 ...
 */
double NOERestraint::unprotected_evaluate(DerivativeAccumulator *accum) const {
  core::XYZ d0(p0_);
  core::XYZ d1(p1_);
