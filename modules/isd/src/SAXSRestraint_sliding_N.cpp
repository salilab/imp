/**
 *  \file SAXSRestraint_sliding_N.h
...
#include <IMP/isd/SAXSRestraint_sliding_N.h>
...
SAXSRestraint_sliding_N::SAXSRestraint_sliding_N(const Particles& particles, const Scale& gamma,
...
ParticlesTemp SAXSRestraint_sliding_N::get_input_particles() const
{
...
ContainersTemp SAXSRestraint_sliding_N::get_input_containers() const
{
...
void SAXSRestraint_sliding_N::compute_profile(saxs::Profile& model_profile) {
...
/** \param[in] acc If true (not NULL), partial first derivatives should be
    ...
    \return score associated with this restraint for the given state of
            the model.
*/
double SAXSRestraint_sliding_N::unprotected_evaluate(DerivativeAccumulator *acc) const
{
  IMP_LOG(TERSE, "SAXSRestraint_sliding_N::unprotected_evaluate\n");

  /* compute Icalc */
  saxs::Profile model_profile(exp_profile_.get_min_q(),
                            exp_profile_.get_max_q(),
                            exp_profile_.get_delta_q());
  const_cast<SAXSRestraint_sliding_N*>(this)->compute_profile(model_profile);
  ...
  for (unsigned int iq=0; iq<profile_size; iq++) {
      ...
      double val = Iexp - gamma_val*Icalc;
      score += 0.5*log(sigmaq[iq]) + val*val/(2*sigmaq[iq]);
  }
...
void SAXSRestraint_sliding_N::do_show(std::ostream&) const
{
//   out << "SAXSRestraint_sliding_N: for " << particles_.size() << " particles "
