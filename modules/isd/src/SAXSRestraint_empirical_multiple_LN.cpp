/**
 *  \file SAXSRestraint_empirical_multiple_LN.h
...
#include <IMP/isd/SAXSRestraint_empirical_multiple_LN.h>
...
SAXSRestraint_empirical_multiple_LN::SAXSRestraint_empirical_multiple_LN(const Particles& particles, const Scales& sigmas,
        const Scale& gamma, const saxs::Profile& exp_profile, saxs::FormFactorType ff_type) :
  sigmas_(sigmas), gamma_(gamma),  exp_profile_(exp_profile), ff_type_(ff_type) {
...
ParticlesTemp SAXSRestraint_empirical_multiple_LN::get_input_particles() const
{
...
ContainersTemp SAXSRestraint_empirical_multiple_LN::get_input_containers() const
{
...
void SAXSRestraint_empirical_multiple_LN::compute_profile(saxs::Profile& model_profile) {
...
/** \param[in] acc If true (not NULL), partial first derivatives should be
    ...
    \return score associated with this restraint for the given state of
            the model.
*/
double SAXSRestraint_empirical_multiple_LN::unprotected_evaluate(DerivativeAccumulator *acc) const
{
  IMP_LOG(TERSE, "SAXSRestraint_empirical_multiple_LN::unprotected_evaluate\n");

  /* compute Icalc */
  saxs::Profile model_profile(exp_profile_.get_min_q(),
                            exp_profile_.get_max_q(),
                            exp_profile_.get_delta_q());
  const_cast<SAXSRestraint_empirical_multiple_LN*>(this)->compute_profile(model_profile);
  ...
  for (unsigned int iq=0; iq<profile_size; iq++) {
      double sigma_val=sigmas_[iq].get_scale();
...
void SAXSRestraint_empirical_multiple_LN::do_show(std::ostream&) const
{
//   out << "SAXSRestraint_empirical_multiple_LN: for " << particles_.size() << " particles "
