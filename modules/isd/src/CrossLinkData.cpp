...
CrossLinkData::CrossLinkData(
        Floats dist_grid, Floats omega_grid, Floats sigma_grid, double lexp,
        double don, double doff, int prior_type) :
...
// switching cumul to zero between don and doff
  if(dist>doff){cumul=0.;}
  if(dist>don && dist<=doff){
   cumul*= pow(doff*doff-dist*dist,2) * (doff*doff+2.*dist*dist-3.*don*don)
...
CrossLinkData::CrossLinkData(Floats dist_grid,
                          Floats omega_grid, Floats sigma_grid,
  ...
  base::Object("Data Structure for CrossLinkMSRestraint %1%")
{
 prior_type_ = prior_type;
 bias_=true;
//this constructor calculates the marginal likelihood using a biasing potential
...
Floats CrossLinkData::get_nonmarginal_elements(double sigmai, Floats dists)
                                                             const {
     Floats probs;
     if (bias_){
        for (unsigned n=0;n<dists.size();n++){
...
Floats CrossLinkData::get_marginal_elements(double sigma, Floats dists)
                                                             const {
    Floats probs;
...
// value of sigma defined in the grid
   ...
   double sigma_min = *( std::min_element( sigma_grid_.begin(),
                                             sigma_grid_.end() ) );
...
int CrossLinkData::get_closest(std::vector<double> const& vec, double value)
                                                            const {
...
double CrossLinkData::get_unbiased_element(double dist,
                                                       double sigmai) const
{
...
double CrossLinkData::get_biased_element(double dist,
                                                       double sigmai) const
{
