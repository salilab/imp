/**
 *  \file SAXSRestraint_sliding_LN.h
...
#include <IMP/isd/SAXSRestraint_sliding_LN.h>
...
SAXSRestraint_sliding_LN::SAXSRestraint_sliding_LN(const Particles& particles, const Scale& gamma,
        const Scale& alphag, const Scale& alphab, const Scale& q0,
        const saxs::Profile& exp_profile, saxs::FormFactorType ff_type) :
  gamma_(gamma), alphag_(alphag), alphab_(alphab), q0_(q0), exp_profile_(exp_profile), ff_type_(ff_type) {
...
ParticlesTemp SAXSRestraint_sliding_LN::get_input_particles() const
{
  ...
  pts.push_back(alphag_);
  pts.push_back(alphab_);
  pts.push_back(q0_);
  return pts;
}


ContainersTemp SAXSRestraint_sliding_LN::get_input_containers() const
{
...
void SAXSRestraint_sliding_LN::compute_profile(saxs::Profile& model_profile) {
...
/** \param[in] acc If true (not NULL), partial first derivatives should be
    ...
    \return score associated with this restraint for the given state of
            the model.
*/
double SAXSRestraint_sliding_LN::unprotected_evaluate(DerivativeAccumulator *acc) const
{
  IMP_LOG(TERSE, "SAXSRestraint_sliding_LN::unprotected_evaluate\n");

  /* compute Icalc */
  saxs::Profile model_profile(exp_profile_.get_min_q(),
                            exp_profile_.get_max_q(),
                            exp_profile_.get_delta_q());
  const_cast<SAXSRestraint_sliding_LN*>(this)->compute_profile(model_profile);

  // compute Scales
  std::vector<double> sigmaq;
  double ag=alphag_.get_scale();
  double ab=alphab_.get_scale();
  double q0val=q0_.get_scale();
  ...
  for (unsigned int iq=0; iq<profile_size; iq++) {
      ...
      double q = exp_profile_.get_q(iq);
      if (q <= q0val) {
          sigmaq.push_back(err*err/(q0val*ag));
      }else{
          sigmaq.push_back(err*err/(q*ab));
      }
  }
  ...
  for (unsigned int iq=0; iq<profile_size; iq++) {
      ...
      double val = log(Iexp/(gamma_val*Icalc));
      score += 0.5*log(sigmaq[iq]*Iexp*Iexp) + val*val/(2*sigmaq[iq]);
  }
...
void SAXSRestraint_sliding_LN::do_show(std::ostream&) const
{
//   out << "SAXSRestraint_sliding_LN: for " << particles_.size() << " particles "
