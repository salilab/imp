...
namespace {
  typedef algebra::internal::MinimalSet<double,
          Particle*, std::greater<double> > SingletonMaximumMS;
  template <class It, class F>
  SingletonMaximumMS find_minimal_set_SingletonMaximum(It b, It e, F *f,
                                                         unsigned int n) {
    IMP_LOG(TERSE, "Finding Maximum " << n << " of "
            << std::distance(b,e) << std::endl);
    SingletonMaximumMS bestn(n);
    for (It it= b; it != e; ++it) {
...
::unprotected_evaluate(DerivativeAccumulator *da) const {
  SingletonMaximumMS bestn
    = find_minimal_set_SingletonMaximum(c_->particles_begin(),
                                         c_->particles_end(),
                                         f_.get(), n_);
