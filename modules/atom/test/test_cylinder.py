...
class DistanceTests(IMP.test.TestCase):
    ...
    def test_constructions(self):
        ...
        v=V(0,0,0)
        s=S(V(-1,1,1), V(1,1,1))
        ss= IMP.atom.shortest_segment(s,v)
        ss.first.show()
        ss.second.show()
        ...
        ss= IMP.atom.shortest_segment(s, s2)
        print "show last"
        ss.first.show()
        ss.second.show()
    ...
    def test_random(self):
        ...
        b00= IMP.atom.BondedDecorator.create(p00)
        p01= IMP.Particle(m)
        xyz01= IMP.core.XYZDecorator.create(p01)
        b01= IMP.atom.BondedDecorator.create(p01)
        b0= IMP.atom.custom_bond(b00, b01, 1)
        p10= IMP.Particle(m)
        xyz10= IMP.core.XYZDecorator.create(p10)
        b10= IMP.atom.BondedDecorator.create(p10)
        p11= IMP.Particle(m)
        xyz11= IMP.core.XYZDecorator.create(p11)
        b11= IMP.atom.BondedDecorator.create(p11)
        b1= IMP.atom.custom_bond(b10, b11, 1)
        l= IMP.core.Linear(0,1)
        ps= IMP.atom.BondBondPairScore(l)
        for i in range(0,1):
            ...
            s1= S(IMP.algebra.random_vector_in_box(V(0,0,0), V(100,100,100)),
                  IMP.algebra.random_vector_in_box(V(0,0,0), V(100,100,100)))
            ss= IMP.atom.shortest_segment(s0, s1)
