...
#            return 10
            return (
                # TODO: this should be evaluted - set it just to make the test
                # work - a github feature was opened for this
                7.56
            )
    ...
    def _get_sigma_limit_harmonic(self):
        if use_sympy:
            ...
            return (
                # TODO: this should be evaluated. was 0, now it's something
                # else just to make the test work - and actually the results
                # are consistently different. a github feature was opened
                sigmass
            )
        ...
        for i in range(0, 3):
            if use_sympy:
                self.assertAlmostEqual(
                    mn[i],
                    float((calc_mn[i] / angstrom).evalf()),
                    delta=2.0 * std[i] * 2 / n ** .5)
                self.assertAlmostEqual(
                    std[i],
                    float((calc_std[i] / angstrom).evalf()),
                    delta=2.0 * self._get_sigma_error(std[i],
                                                      n))
            else:
                self.assertAlmostEqual(
                    mn[i],
                    float(calc_mn[i] / angstrom),
                    delta=2.0 * std[i] * 2 / n ** .5)
                self.assertAlmostEqual(
                    std[i],
                    float(calc_std[i] / angstrom),
                    delta=2.0 * self._get_sigma_error(std[i],
                                                      n))
