...
class SoapBondSeparation {
  ...
  struct SoapLibraryBond {
    ...
    SoapLibraryBond(ResidueType ir, AtomType ia1,
                    AtomType ia2) : r(ir), a1(ia1), a2(ia2) {}
  };
  ...
  struct Bridge {
    ...
    Bridge(Residue ir1, Residue ir2, Atom ia1,
           Atom ia2) : r1(ir1), r2(ir2), a1(ia1), a2(ia2) {}
  };
  ...
  Bridges get_disulfide_bridges(Residue r1, Residue r2) const {
    ...
    for (NearbyCys::const_iterator i1 = cys1.begin(); i1 != cys1.end(); ++i1) {
      for (NearbyCys::const_iterator i2 = cys2.begin();
           i2 != cys2.end(); ++i2) {
        if (i1->second != i2->second
            && std::abs(i1->second - ind_r1)
               + std::abs(i2->second - ind_r2) <= ssseq) {
  ...
  void fill_residue_type_map(const SoapLibraryBonds &bonds) {
    for (SoapLibraryBonds::const_iterator it = bonds.begin();
         it != bonds.end(); ++it) {
  ...
  int get_bond_separation_same_chain(Residue r1, Residue r2,
                                     const SoapResidueBond &rb1,
                                     const SoapResidueBond &rb2,
                                     int at1, int at2) const {
    bool sameres;
    int bonddist = get_bond_separation_same_chain_noss(r1, r2, rb1, rb2,
                                                       at1, at2, sameres);
    ...
    if (disulfide_ && cys_residue_bond_ && !sameres) {
      ...
      for (Bridges::const_iterator it = bridges.begin(); it != bridges.end();
           ++it) {
        int ss_dist = get_bond_separation_same_chain_noss(r1, it->r1, rb1,
                               *cys_residue_bond_, at1, sg_atom_type_, sameres)
                      + get_bond_separation_same_chain_noss(it->r2, r2,
                               *cys_residue_bond_, rb2, sg_atom_type_,
                               at2, sameres) + 1;
