...
CHARMMResidueTopology *get_two_patch_residue(std::string &name,
  ...
  IMP_THROW("Patching residue atom " << name << " does not start with 1 or 2",
            ValueException);
}

std::pair<CHARMMResidueTopology *, CHARMMAtomTopology> handle_two_patch_atom(
    const CHARMMAtomTopology &atom, CHARMMResidueTopology *res1,
    CHARMMResidueTopology *res2) {
...
Atom CHARMMBondEndpoint::get_atom(
    const CHARMMResidueTopology *current_residue,
    const CHARMMResidueTopology *previous_residue,
    const CHARMMResidueTopology *next_residue,
...
void CHARMMIdealResidueTopology::remove_atom(std::string name) {
  ...
  bonds_.erase(
      std::remove_if(bonds_.begin(), bonds_.end(), bond_has_atom<2>(name)),
      bonds_.end());
  angles_.erase(
      std::remove_if(angles_.begin(), angles_.end(), bond_has_atom<3>(name)),
      angles_.end());
  dihedrals_.erase(std::remove_if(dihedrals_.begin(), dihedrals_.end(),
                                  bond_has_atom<4>(name)),
                   dihedrals_.end());
...
void check_empty_patch(const CHARMMPatch *patch) {
  if (patch->get_number_of_atoms() == 0 &&
      ...
      patch->get_number_of_bonds() == 0 && patch->get_number_of_angles() == 0 &&
...
void CHARMMPatch::apply(CHARMMResidueTopology *res1,
  ...
  for (unsigned int i = 0; i < get_number_of_bonds(); ++i) {
    ...
    res->add_bond(
        CHARMMBond(handle_two_patch_bond(get_bond(i), res1, res2, res)));
  }
  for (unsigned int i = 0; i < get_number_of_angles(); ++i) {
    CHARMMResidueTopology *res =
        get_two_patch_residue_for_bond(get_angle(i), res1, res2);
    res->add_angle(
        CHARMMAngle(handle_two_patch_bond(get_angle(i), res1, res2, res)));
  }
  for (unsigned int i = 0; i < get_number_of_dihedrals(); ++i) {
    CHARMMResidueTopology *res =
        get_two_patch_residue_for_bond(get_dihedral(i), res1, res2);
    res->add_dihedral(CHARMMDihedral(
        handle_two_patch_bond(get_dihedral(i), res1, res2, res)));
  }
  for (unsigned int i = 0; i < get_number_of_impropers(); ++i) {
    CHARMMResidueTopology *res =
        get_two_patch_residue_for_bond(get_improper(i), res1, res2);
    res->add_improper(CHARMMDihedral(
        handle_two_patch_bond(get_improper(i), res1, res2, res)));
  }
  ...
  for (unsigned int i = 0; i < get_number_of_internal_coordinates(); ++i) {
    ...
    CHARMMResidueTopology *res =
        get_two_patch_residue_for_bond(get_internal_coordinate(i), res1, res2);
    res->add_internal_coordinate(CHARMMInternalCoordinate(
        handle_two_patch_bond(ic, res1, res2, res), ic.get_first_distance(),
        ic.get_first_angle(), ic.get_dihedral(), ic.get_second_angle(),
...
struct ModelInternalCoordinate {
  ...
  bool improper;
  Atoms atoms;
  ModelInternalCoordinate(const CHARMMInternalCoordinate &ic, Atoms new_atoms)
      : atoms(new_atoms) {
...
void fill_internal_coordinates(base::Vector<ModelInternalCoordinate> &ics,
  ...
  for (base::Vector<ModelInternalCoordinate>::iterator it = ics.begin();
    ...
    if (it->first_angle == 0.) {
      if (it->improper) {
        it->first_angle =
            fill_angle(it->atoms[0], it->atoms[2], it->atoms[1], ff);
      } else {
        it->first_angle =
            fill_angle(it->atoms[0], it->atoms[1], it->atoms[2], ff);
      }
    }
    if (it->second_angle == 0.) {
      it->second_angle =
          fill_angle(it->atoms[1], it->atoms[2], it->atoms[3], ff);
    }
  }
}
...
bool build_cartesian_from_internal(const ModelInternalCoordinate &ic) {
  if (core::XYZ::particle_is_instance(ic.atoms[1]) &&
    ...
    if (!core::XYZ::particle_is_instance(ic.atoms[3]) &&
      ...
      if (r != 0. && theta != 0.) {
        build_cartesian(ic.atoms[0], ic.atoms[1], ic.atoms[2], ic.atoms[3], r,
                        phi, theta);
        return true;
      }
      ...
      if (r != 0. && theta != 0.) {
        if (ic.improper) {
          build_cartesian(ic.atoms[3], ic.atoms[1], ic.atoms[2], ic.atoms[0], r,
                          -phi, theta);
        } else {
          build_cartesian(ic.atoms[3], ic.atoms[2], ic.atoms[1], ic.atoms[0], r,
                          phi, theta);
        }
        return true;
      }
  ...
  for (base::Vector<ModelInternalCoordinate>::const_iterator it = ics.begin();
    ...
    if (!it->improper) {
      if (seed_triplet(it->atoms[0], it->atoms[1], it->atoms[2], ics, seed) ||
          seed_triplet(it->atoms[1], it->atoms[2], it->atoms[3], ics, seed)) {
        return true;
      }
    }
  }
...
unsigned count_atoms_with_coordinates(
    const CHARMMSegmentTopology *seg,
    const std::map<const CHARMMResidueTopology *, Hierarchy> &resmap) {
  unsigned int ct = 0;
...
unsigned int assign_remaining_coordinates(
    const CHARMMSegmentTopology *seg,
    const std::map<const CHARMMResidueTopology *, Hierarchy> &resmap,
...
void CHARMMTopology::add_coordinates(Hierarchy hierarchy) const {
  ...
  for (CHARMMSegmentTopologyConstIterator segit = segments_begin();
    ...
    if (nremain > 0) {
      IMP_WARN("The Cartesian coordinates of "
               << nremain
