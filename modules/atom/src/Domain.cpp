...
////////////////////////////// DEPRECATED
Domain Domain::setup_particle(Model *m, ParticleIndex pi,
...
Domain Domain::setup_particle(IMP::kernel::ParticleAdaptor decorator,
                              Int residues_begin, Int residues_end) {
  IMP_USAGE_CHECK(!get_is_setup(decorator.get_model(),
                                decorator.get_particle_index()), "Particle "
                  << decorator.get_model()
                  ->get_particle_name(decorator.get_particle_index())
                  << " already set up as Domain");
  do_setup_particle(decorator.get_model(),
