...
RMSDCalculator::RMSDCalculator(const ParticlesTemp& ps) {
  ...
  for (core::XYZs::const_iterator it = xyzs.begin(); it != xyzs.end(); ++it) {
...
double RMSDCalculator::get_squared_rmsd(const algebra::Transformation3D& t1,
  ...
  for (unsigned int i = 0; i < 3; i++) {
    for (unsigned int j = i; j < 3; j++) {
      if (i == j)
        ret += (2 - 2 * m[i][i]) * d_[i][i];
      else
        ret += 2 * (-m[i][j] - m[j][i]) * d_[i][j];
    }
  }
  algebra::Vector3D translation_diff =
      t1.get_translation() - t2.get_translation();
