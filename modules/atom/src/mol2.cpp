...
std::string mol2_string(Atom a) {
  ...
  mol2_file << std::setw(1) << get_element_table().get_name(a.get_element())[0];
  ...
  mol2_file.setf(std::ios::right, std::ios::adjustfield);
  mol2_file << std::setw(14) << std::setprecision(4) << Charged(a).get_charge()
            << std::endl;
...
std::string mol2_string(Bond b, unsigned int count) {
  ...
  if (Atom::get_is_setup(pa) && Atom::get_is_setup(pb)) {
    ...
    IMP_THROW("bonded particle(s) is not atom " << *pa << " " << *pb,
              IOException);
  }
  return oss.str();
}
std::string mol2_string(Residue r) {
  ...
  oss << get_by_type(r, ATOM_TYPE).size() << " " << get_internal_bonds(r).size()
      << std::endl;
...
void bond_particle(kernel::Model*, const String& mol2_bondline,
  ...
  if (bond_type.compare("1") == 0) {
    ...
    IMP_THROW("Error in bond types on " << mol2_bondline, IOException);
  }
...
Hierarchy read_molecule_mol2(kernel::Model* model, std::istream& mol2_file,
  ...
  Hierarchy molecule_d = molecule_particle(model, molecule_name, molecule_type);
