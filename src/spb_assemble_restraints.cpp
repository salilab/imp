...
// Excluded volume
//
add_SPBexcluded_volume(m,allrs,all_mol,mydata.GFP_exc_volume,mydata.kappa_vol);
//
// Symmetry
...
if(mydata.add_IL2_layer){
 add_bayesian_layer_restraint(m,allrs, IL2_ps,
 ISD_ps["GAP_A"], ISD_ps["GAP_B"]);
}
//
// TILT restraint
//
...
if(mydata.add_tilt){
 ...
 if(mydata.protein_list["Spc110p"]){
   add_tilt(m,allrs,all_mol[0],"Spc110p",
   IntRange(700,705),mydata.tilt,kappa_tilt);
 }
 if(mydata.protein_list["Cnm67p"]){
   add_tilt(m,allrs,all_mol[0],"Cnm67p",
   IntRange(1,573),mydata.tilt,kappa_tilt);
 }
...
if(mydata.protein_list["Spc42p"]){
 // last bead before CC
 add_stay_on_plane_restraint(m,allrs,all_mol[0],"Spc42p",50,  mydata.kappa);
 // first bead after CC
...
// if adding to single terminus bead
 add_stay_on_plane_restraint(m,allrs,all_mol[0],"Spc42p",360, mydata.kappa);
...
// IL2
 y2h->add_restraint(y2h_restraint(m,all_mol[0], "Cnm67p", IntRange(442,573),
...
// Not technically a Y2H restraint but similar
 y2h->add_restraint(y2h_restraint(m,all_mol[0], "Cnm67p", IntRange(502,507),
...
//
if(mydata.protein_list["Spc110p"]){
 add_stay_on_plane_restraint(m,allrs,all_mol[0],"Spc110p",900, mydata.kappa);
 add_stay_on_plane_restraint(m,allrs,all_mol[0],"Spc110p",940, mydata.kappa);
}
//
...
//
if(mydata.add_GFP){
  add_link(m,allrs,all_mol[0],      "Spc110p", "C",
             all_mol,   "Spc110p-C-GFP", "N", mydata.kappa);
  add_link(m,allrs,all_mol[0],        "Cmd1p", "N",
             all_mol,     "Cmd1p-N-GFP", "C", mydata.kappa);
  add_link(m,allrs,all_mol[0],        "Cmd1p", "C",
             all_mol,     "Cmd1p-C-GFP", "N", mydata.kappa);
  add_link(m,allrs,all_mol[0],       "Spc42p", "N",
             all_mol,    "Spc42p-N-GFP", "C", mydata.kappa);
  add_link(m,allrs,all_mol[0],       "Spc42p", "C",
             all_mol,    "Spc42p-C-GFP", "N", mydata.kappa);
  add_link(m,allrs,all_mol[0],       "Spc29p", "N",
             all_mol,    "Spc29p-N-GFP", "C", mydata.kappa);
  add_link(m,allrs,all_mol[0],       "Spc29p", "C",
             all_mol,    "Spc29p-C-GFP", "N", mydata.kappa);
  add_link(m,allrs,all_mol[0],       "Cnm67p", "C",
