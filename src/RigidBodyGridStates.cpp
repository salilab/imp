...
Ints RigidBodyGridStates::get_index(unsigned int index) const {
  ...
  i[3] = (((index - i[0]) / nbin_[0] - i[1]) / nbin_[1] - i[2]) / nbin_[2] %
         nbin_[3];
  i[4] = ((((index - i[0]) / nbin_[0] - i[1]) / nbin_[1] - i[2]) / nbin_[2] -
          i[3]) /
         nbin_[3] % nbin_[4];
  i[5] = (((((index - i[0]) / nbin_[0] - i[1]) / nbin_[1] - i[2]) / nbin_[2] -
           i[3]) /
              nbin_[3] -
          i[4]) /
         nbin_[4];
...
algebra::ReferenceFrame3D RigidBodyGridStates::get_reference_frame(
  ...
  algebra::Rotation3D rotz = algebra::get_rotation_about_axis(
      algebra::Vector3D(0, 0, 1), double(nindex[3]) * binsize_[3] + begin_[3]);
  algebra::Rotation3D tilt = algebra::get_rotation_about_axis(
      algebra::Vector3D(0, 1, 0), double(nindex[4]) * binsize_[4] + begin_[4]);
  algebra::Rotation3D swing = algebra::get_rotation_about_axis(
      algebra::Vector3D(0, 0, 1), double(nindex[5]) * binsize_[5] + begin_[5]);
  ...
  algebra::ReferenceFrame3D state =
      algebra::ReferenceFrame3D(algebra::Transformation3D(
          algebra::compose(
              swing, algebra::compose(tilt, algebra::compose(rotz, rot0))),
          trans));
  return state;
}
...
void RigidBodyGridStates::load_particle_state(unsigned int i,
                                              Particle *p) const {
