...
#define DOPE_TYPE(NameKey) const DopeType \
    NameKey(DopeType::add_key(#NameKey));
#define DOPE_TYPE_ALIAS(Name, Key) const DopeType Name\
  (DopeType::add_alias(DopeType(#Key), #Name));
    ...
    DOPE_TYPE(ILE_CG2)
    DOPE_TYPE(ILE_CD)
    DOPE_TYPE_ALIAS(ILE_CD1, ILE_CD)
    DOPE_TYPE(LEU_N)
    DOPE_TYPE(LEU_CA)
    DOPE_TYPE(LEU_C)
    DOPE_TYPE(LEU_O)
...
void add_dope_score_data(atom::Atom atom) {
  int type;
  atom::Residue rd= get_residue(atom);
  ...
  if (type==-1 && atom.get_element() != atom::H) {
...
void add_dope_score_data(atom::Hierarchy h) {
  atom::HierarchiesTemp atoms= atom::get_by_type(h, atom::ATOM_TYPE);
  for (unsigned int i= 0; i< atoms.size(); ++i) {
    add_dope_score_data(atom::Atom(atoms[i]));
  }
}
