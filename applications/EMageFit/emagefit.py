...
def setup_sampling_schema(model, params):
    ...
    sampling_schema = sampling.SamplingSchema(
        n_rbs, params.fixed, params.anchor)
    ...
    for i, rb in enumerate(model.components_rbs):
        model.set_rb_states(
            rb,
            sampling_schema.get_sampling_transformations(i))
...
def create_dockings_from_xlinks(params):
    ...
    for rec, lig in docking_pairs:
        ...
        # refinement. Hexdock therefore would "apply" an identity
        # transformation
