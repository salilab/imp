...
void Gnuplot::print_profile_script(const std::vector<std::string>& pdbs) {
  ...
  plt_file << "set title 'Profiles'\n";
  plt_file << "set xlabel 'q (\305^{-1})';set ylabel 'I(q) log-scale'\n";
  plt_file << "plot ";
...
void Gnuplot::print_canvas_script(
  ...
  plt_file << "set origin 0,0;set size 1,0.3; set tmargin 0; set bmargin 3;"
           << "set ylabel '';set format y '';"
           << "set xtics nomirror font 'Helvetica,18';set ytics nomirror;"
  ...
  for(unsigned int i=0; i<fps.size(); i++) {
    ...
    plt_file <<  ", '" << fit_file_name << "' u 1:3 thru log(y) t '" << pdb_name
             << " " << fps[i].get_chi() << "' w lines lw 2.5 lc rgb '#"
             << hex_color << "'";
  }
  plt_file << std::endl;
  plt_file << "unset multiplot" << std::endl;
