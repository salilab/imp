...
int main(int argc, char** argv) {
  ...
  po::options_description desc("Usage: <pdb1> <pdb2> <trans_file>");
  desc.add_options()(
      "help",
      "compute the SOAP score of the interface between pdb1 and pdb2 \
for a set of transformations in the trans_file.")(
  ...
  IMP::atom::Hierarchy mhd1 = IMP::atom::read_pdb(
      pdb1, model, new IMP::atom::NonWaterNonHydrogenPDBSelector(), true, true);

  IMP::atom::Hierarchy mhd2 = IMP::atom::read_pdb(
      pdb2, model, new IMP::atom::NonWaterNonHydrogenPDBSelector(), true, true);
  IMP::kernel::Particles particles1 = IMP::get_as<IMP::kernel::Particles>(
      get_by_type(mhd1, IMP::atom::ATOM_TYPE));
  IMP::kernel::Particles particles2 = IMP::get_as<IMP::kernel::Particles>(
      get_by_type(mhd2, IMP::atom::ATOM_TYPE));
  ...
  for (unsigned int t = 0; t < transforms.size(); t++) {
      ...
      for (Grid::IndexIterator it = grid.indexes_begin(lb, ub);
        ...
        for (unsigned int vIndex = 0; vIndex < grid[*it].size(); vIndex++) {
          ...
          if (dist2 < distance_threshold2) {
            score += soap.get_score(model, IMP::kernel::ParticleIndexPair(
                                               pis1[r_index], pis2[l_index]),
                                    sqrt(dist2));
          }
        }
      }
    ...
    surface_area =
        s.get_solvent_accessibility(IMP::core::XYZRs(particles), 1.4);
    ...
    for (unsigned int i = 0; i < particles.size(); i++) {
      sas_score +=
          sas_stat.get_score(model, particles[i]->get_index(), surface_area[i]);
    }

    // save
    SOAPResult r(t + 1, sas_score + score, false, sas_score, score,
                 transforms[t]);
    results.push_back(r);
    if ((t + 1) % 1000 == 0)
