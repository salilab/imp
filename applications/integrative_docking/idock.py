...
def parse_args():
    epilog = """Note that in order to run this application, you must also have
PatchDock and FiberDock (available from
...
find 'reduce' in the system path."""
    ...
    parser.add_option('--fiber_dock', metavar='DIR',
                      ...
                      help='Directory where FiberDock tools are installed. '
                           'If not specified, the value of the FIBER_DOCK_HOME '
                           'environment variable is used if set, otherwise the '
                           'tools are assumed to be in the default path.')
