...
Each docked complex will be compared against cross links in cross_links_file.")
  ...
  IMP::atom::Hierarchy mhd =
    IMP::atom::read_pdb(pdb, model,
                        ...
                        true, true);
  IMP::kernel::Particles residue_particles = get_by_type(mhd, IMP::atom::RESIDUE_TYPE);

  // get CA atoms for residues
  IMP::kernel::Particles ca_atoms;
  for(unsigned int i=0; i<residue_particles.size(); i++) {
    IMP::atom::Atom at =
      IMP::atom::get_atom(IMP::atom::Residue(residue_particles[i]),
                          IMP::atom::AT_CA);
  ...
  for(unsigned int i=0; i<cross_links.size(); i++) {
    IMP::algebra::Vector3D v1 =
      get_ca_coordinate(ca_atoms, cross_links[i].get_residue1(),
                        cross_links[i].get_chain1());
    IMP::algebra::Vector3D v2 =
      get_ca_coordinate(ca_atoms, cross_links[i].get_residue2(),
  ...
  std::cerr << "Score = " << score << " unsatisfied_num "
            << unsatisfied_num << std::endl;
  return 0;
}
