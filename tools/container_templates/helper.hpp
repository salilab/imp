...
private:
  ...
  typedef IMP::compatibility::set<VARIABLETYPE> Index;
...
protected:
  ...
  template <class F>
    struct AccIf {
    F f_;
    mutable PLURALVARIABLETYPE rem_;
    AccIf(F f, PLURALVARIABLETYPE &rem): f_(f), rem_(rem){}
    bool operator()(ARGUMENTTYPE cur) const {
      if (f_(cur)) {
        rem_.push_back(cur);
        return true;
      } return false;
