/**
 *  \file TripletDerivativeModifier.h
...
public:
 ...
 /** Apply the function to a single value*/
  virtual void apply(Model *m, const ParticleIndexTriplet& v,
                     DerivativeAccumulator &da) const {
    apply(internal::get_particle(m, v), da);
  }

  /** Apply the function to a collection of ParticleTripletsTemp */
  virtual void apply(Model *m, const ParticleIndexTriplets &o,
                     DerivativeAccumulator &da) const {
    for (unsigned int i=0; i < o.size(); ++i) {
      apply(m, o[i], da);
    }
...
class TripletDerivativeFunctor {
  Pointer<const TripletDerivativeModifier> f_;
  DerivativeAccumulator *da_;
public:
  //! Store the TripletModifier and the optional DerivativeAccumulator
  TripletDerivativeFunctor(const TripletDerivativeModifier *f,
