...
namespace IMP
{
  ...
  /**
     ...
     \return A copy of the functor passed in. Use this if '
     you care about the functor state.
  */
  ...
  /**
     ...
     F must define a type Data which is returned by each each call.
     The result of the parent call is passed as the second argument 
     to the operator() of the child. i.e.
     struct DepthVisitor {
  ...
  */
  ...
  F breadth_first_traversal_with_data(HD d, F f,
  ...
  */
  template <class HD, class F>
  F depth_first_traversal_with_data(HD d,  F f,
                                    typename F::result_type i)
  {
    typedef std::pair<typename F::result_type, HD> DP;
    std::vector<DP> stack;
