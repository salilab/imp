...
g130: u1 = dp + dal - 3.0 * (fp - f) / (ap - alpha);
  u2 = u1 * u1 - dp * dal;
  if (u2 < 0.) {
    u2 = 0.;
  }
...
/* THE MINIMUM HAS BEEN BRACKETED. TEST WHETHER THE TRIAL POINT LIES
  ...
  if (at < (1.01 * std::min(alpha, ap)) || at > (0.99 * std::max(alpha, ap))) {
    at = (alpha + ap) / 2.0;
  }
  goto g150;
...
/* THE MINIMUM HAS NOT BEEN BRACKETED. TEST IF BOTH POINTS ARE
  ...
  SMALLER THAN EITHER. */
g140: if (dal > 0.0 && 0.0 < at && at < (0.99 * std::min(ap, alpha))) {
    goto g150;
  }
...
/* CALCULATE THE DOUBLY UPDATED HESSIAN TIMES THE CURRENT
  ...
  for (i = 0; i < n; i++) {
    u1 -= search[i] * dx[i] / u3;
    u2 += (1.0 + u4 / u3) * search[i] * dx[i] / u3 - destimate[i] * dx[i] / u3;
  }
  for (i = 0; i < n; i++) {
